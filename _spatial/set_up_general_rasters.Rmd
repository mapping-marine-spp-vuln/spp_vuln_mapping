...
* Global rasters at 10 x 10 km and 1 x 1 km resolution Mollweide CRS.
...
if(!file.exists(rast_base_10km_file)) {
  ...
  rast_base_10km <- project(x, crs = moll_crs, res = 10000, progress="text")
...
if(!file.exists(rast_base_1km_file)) {
  rast_base_10km <- rast(rast_base_10km_file)
...
* Use `rasterize` to rasterize the reprojected ocean shapefile to Mollweide CRS with a 1 km resolution.
...
if(!file.exists(ocean_1km_file)) {
  ...
  rast_base_moll_1km <- rast(rast_base_1km_file)
  
  ocean_moll_1km <- rasterize(ocean_moll_sf, rast_base_1km, 
...
if(!file.exists(ocean_10km_file)) {
  ...
  ocean_moll_1km <- rast(ocean_1km_file)
...
* Use `rasterize` to rasterize the reprojected buffered land shapefile to Mollweide CRS with a 1 km resolution.
    * This will be saved on the server, too large for Github.
...
if(!file.exists(coastal_3nmi_buffer_1km_file)) {
  ...
  land_moll_1km <- rasterize(land_3nmi_moll_sf, 
                             rast_base_moll_1km, 
                             field = NULL, ### NULL sets all polygons to 1
                             background = NA)
...
if(!file.exists(coastal_3nmi_10km_file)) {
  ...
  coastal_moll_1km <- rast(coastal_3nmi_buffer_1km_file)
...
if(!file.exists(coastal_1km_buffer_1km_file)) {
  ...
  land_moll_1km <- rasterize(land_1km_moll_sf, 
                             rast_base_moll_1km, 
                             field = NULL, ### NULL sets all polygons to 1
                             background = NA)
...
if(!file.exists(coastal_1km_10km_file)) {
  ...
  coastal_moll_1km <- rast(coastal_1km_buffer_1km_file)
...
coastal_1km_moll_10km <- rast(coastal_1km_10km_file)
...
if(!file.exists(loiczid_rast_file)) {
  ...
  loiczid_rast <- rast(ext = extent(c(xmin = -180, xmax = 180, ymin = -90, ymax = 90)), 
                         res = 0.5,
                         crs = '+init=epsg:4326') %>%
    setValues(1:ncell(.))
  ...
  loiczid_moll <- project(loiczid_rast, ocean_moll_10km, method = 'ngb')
...
loiczid_moll <- rast(loiczid_rast_file)
...
if(!file.exists(meow_rgn_rast_file)) {
  ...
  meow_rast <- rasterize(meow_sf_moll, ocean_moll_10km, 
...
if(!file.exists(bathy_rast_file)) {
  ...
  coastal_rast <- rast(coastal_3nmi_10km_file)
...
```{r}
fao_rast_f <- here('_spatial/fao_mol.tif')

if(!file.exists(fao_rast_f)) {
  fao_shp_f <- here_anx('../fao_rgns/World_Fao_Zones.shp')
  fao_rgns <- read_sf(fao_shp_f) %>%
    smoothr::densify(max_distance = .5) %>% 
      ### a vertex every .5 degrees ought to do it
    st_transform(st_crs(loiczid_moll))
  
  fao_rast <- rasterize(fao_rgns, loiczid_moll, field = 'zone')
  
  writeRaster(fao_rast, fao_rast_f)
}

fao_rast <- rast(fao_rast_f)
plot(fao_rast)
```
