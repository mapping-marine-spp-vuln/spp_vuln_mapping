...
system.time({
  unsup_rf <- randomForest(x = traits_dropna %>% 
                             ### drop two least important vars
                             select(-terr_score, -vert_mig_score) %>%
                             # sample_n(2000) %>%
                             select(where(is.numeric)),
                           mtry = 3, ntree = 1000, proximity = TRUE)
})
...
silh_df_ward1 <- loop_over_clusts(HC = rf_hclust_ward1,
                                  DM = dissim_mtx)
p1 <- ggplot(silh_df_ward1, aes(x = k, y = silh)) +
  geom_point() +
...
silh_df_ward2 <- loop_over_clusts(HC = rf_hclust_ward2,
                                  DM = dissim_mtx)
p2 <- ggplot(silh_df_ward2, aes(x = k, y = silh)) +
  geom_point() +
...
silh_df_compl <- loop_over_clusts(HC = rf_hclust_compl,
                                  DM = dissim_mtx)
p3 <- ggplot(silh_df_compl, aes(x = k, y = silh)) +
  geom_point() +
...
silh_df_singl <- loop_over_clusts(HC = rf_hclust_singl,
                                  DM = dissim_mtx)
p4 <- ggplot(silh_df_singl, aes(x = k, y = silh)) +
  geom_point() +
