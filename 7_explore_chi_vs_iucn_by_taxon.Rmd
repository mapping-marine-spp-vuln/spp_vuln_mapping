...
df_for_blr_chi <- chi_threatened_valid %>%
  select(sciname, taxon, phylum, class, atrisk, threatened, 
         ...
         starts_with('inv_log'), starts_with('range'),
         mob, len, wcol, trp) %>%
  distinct() %>%
...
mdl1 <- glm(threatened ~ chi_npp_cst + impact_npp_cv_cst + inv_log_r_cst, 
           data = df_for_blr_chi,
           family = 'binomial')
summary(mdl1)

### filter for smaller-ranged spp, coastal ranges
mdl2 <- glm(threatened ~ chi_npp_cst + impact_npp_cv_cst + inv_log_r_cst, 
           data = df_for_blr_chi %>% filter(qtile_r == 1),
           family = 'binomial')
summary(mdl2)

### similar but total ranges
mdl3 <- glm(threatened ~ chi_npp_tot + impact_npp_cv_tot + inv_log_r_tot, 
           data = df_for_blr_chi %>% filter(qtile_r == 1),
           family = 'binomial')
summary(mdl3)

### total ranges but filtering for even smaller-ranged spp (qtile 1 ends at 148k km^2)
mdl4 <- glm(threatened ~ chi_npp_tot + impact_npp_cv_tot + inv_log_r_tot, 
           data = df_for_blr_chi %>% filter(range_tot_km2 <= 100000),
           family = 'binomial')
summary(mdl4)

### NPP-weighted mean CHI, drop CV term, keep smallest 20% ranges
mdl5 <- glm(threatened ~ chi_npp_tot + inv_log_r_tot, 
           data = df_for_blr_chi %>% 
             filter(qtile_r <= 1),
...
mdl6 <- glm(threatened ~ chi_rel_tot + inv_log_r_tot, 
           data = df_for_blr_chi %>% 
             filter(qtile_r <= 1),
             # filter(range_tot_km2 <= 100000),
             # filter(range_tot_km2 <= 200000),
...
mdl5a <- glm(threatened ~ chi_npp_tot + inv_log_r_tot, 
           data = df_for_blr_chi %>% 
             filter(qtile_r <= 4),
             # filter(range_tot_km2 <= 100000),
             # filter(range_tot_km2 <= 200000),
