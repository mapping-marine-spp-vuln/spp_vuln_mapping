...
1.  Identify a set of sample cells for analysis (let's go with 100,000).  
...
2.  For each trait (max body length, trophic level, adult mobility, position in water column), for each sample cell:
    * Shuffle all existing trait values, sampling with replacement bootstrap-style - this keeps distribution largely identical but allows some variation
...
3.  Repeat step 2 many many times, Monte Carlo style.
    * For each pixel, calculate distribution (mean, std dev, 95% CI):
        * Distribution of FR and FV around "true" value ("true" meaning scores based on determined trait set).
...
Choose 100,000 cells randomly and filter our full cell data.frame down to just these cells.
...
shuffle_trait <- function(df, trait, traits_df = spp_traits_df) {
  ...
  df2 <- df %>%
    ...
    group_by(cell_id) %>%
    mutate(tmp = sample(tmp, size = n(), replace = TRUE)) %>%
    ungroup() %>%
    rename(!!trait := tmp)
  ...
  if(!file.exists(out_f)) {
    ...
    zxcv <- parallel::mclapply(
      ...
      FUN = function(j) {
        ...
        ### this ensures repeatability if needing to re-run any iteration/trait set
    ...
    fd_result_df <- bind_tmp_fs(tmp_fs, n = n_sims) ### includes error checks
...
Because of the processing demands, let's just do 200 sims of each of these.  This should still give us a reasonable estimate of 95% confidence intervals.
...
for(i in seq_along(trait_vec)) {
  ...
  for(j in seq_along(strs_unif)) {
    ...
    if(!file.exists(out_f)) {
      ...
      str_impact_tmp_df <- bind_tmp_fs(tmp_fs, n = n_sims_imp) ### includes error checks
  ...
  if(!file.exists(out_f)) {
    ...
    tmp_fs <- list.files(dirname(tmp_out_fstem), 
     ...
     if(length(tmp_fs) > 0) {
      ...
      tmp_vec <- (1:n_sims_imp)[-tmp_done]
    } else tmp_vec <- 1:n_sims_imp
    message('Iterating bycatch over ', length(tmp_vec), ' iterations...')
        ...
        system.time({
          spp_fe_imp_cells <- spp_fe_cells %>%
            ...
            .[ , .(imp_mean = sum(cell_imp * fv) / sum(fv)),
               by = cell_id]
         }) ### join steps ~ 40 sec, summarize steps ~ 90 seconds: ~ 2 min
