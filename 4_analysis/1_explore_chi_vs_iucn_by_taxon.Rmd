...
if(!file.exists(spp_chi_file)) { ### unlink(spp_chi_file)
  ...
  spp_ranges <- spp_imp_raw_df %>%
    select(f, range_a_km2, range) %>%
    distinct() %>%
    spread(range, range_a_km2) %>%
    rename(range_cst_km2 = coastal, range_tot_km2 = full)
  
  spp_npp_ranges <- spp_imp_raw_df %>%
    select(f, npp_range_km2, range) %>%
    distinct() %>%
    spread(range, npp_range_km2) %>%
    rename(npp_range_cst_km2 = coastal, npp_range_tot_km2 = full)

  spp_ln_npp_ranges <- spp_imp_raw_df %>%
    select(f, ln_npp_range_km2, range) %>%
    distinct() %>%
    spread(range, ln_npp_range_km2) %>%
    rename(ln_npp_range_cst_km2 = coastal, ln_npp_range_tot_km2 = full)
  ...
  spp_npp_intensity_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, int_npp_tot, range) %>%
    spread(range, int_npp_tot) %>%
    rename(int_npp_tot = full, int_npp_cst = coastal)
    
  spp_npp_intens_cv_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, int_npp_cv, range) %>%
    spread(range, int_npp_cv) %>%
    rename(int_npp_cv_tot = full, int_npp_cv_cst = coastal)

  spp_ln_npp_intensity_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, int_ln_npp_tot, range) %>%
    spread(range, int_ln_npp_tot) %>%
    rename(int_ln_npp_tot = full, int_ln_npp_cst = coastal)
    
  spp_ln_npp_intens_cv_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, int_ln_npp_cv, range) %>%
    spread(range, int_ln_npp_cv) %>%
    rename(int_ln_npp_cv_tot = full, int_ln_npp_cv_cst = coastal)
  ...
  spp_impacts_ln_npp_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, imp_ln_npp_tot, range) %>%
    spread(range, imp_ln_npp_tot) %>%
    rename(impact_ln_npp_tot = full, impact_ln_npp_cst = coastal)
    
  spp_ln_npp_cv_df <- spp_imp_raw_df %>%
    select(f, vuln, stressor, imp_ln_npp_cv, range) %>%
    spread(range, imp_ln_npp_cv) %>%
    rename(impact_ln_npp_cv_tot = full, impact_ln_npp_cv_cst = coastal)
  ...
  spp_imp_cv_df <- full_join(spp_impacts_df, spp_cv_df, 
    ...
    full_join(spp_impacts_ln_npp_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_ln_npp_cv_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_intensity_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_intens_cv_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_npp_intensity_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_npp_intens_cv_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_ln_npp_intensity_df, by = c('f', 'vuln', 'stressor')) %>%
    full_join(spp_ln_npp_intens_cv_df, by = c('f', 'vuln', 'stressor')) %>%
    left_join(spp_ranges, by = 'f') %>%
    left_join(spp_npp_ranges, by = 'f') %>%
    left_join(spp_ln_npp_ranges, by = 'f') %>%
    ...
    select(sciname, iucn_sid, map_src, vuln, stressor, 
           ...
           impact_ln_npp_cst, impact_ln_npp_tot, 
           impact_ln_npp_cv_cst, impact_ln_npp_cv_tot,
           intensity_cst, intensity_tot, 
           intensity_cv_cst, intensity_cv_tot,
           int_npp_cst, int_npp_tot, 
           int_npp_cv_cst, int_npp_cv_tot,
           int_ln_npp_cst, int_ln_npp_tot, 
           int_ln_npp_cv_cst, int_ln_npp_cv_tot,
           range_cst_km2, range_tot_km2,
           npp_range_cst_km2, npp_range_tot_km2,
           ln_npp_range_cst_km2, ln_npp_range_tot_km2) %>%
    distinct() %>%
...
spp_chi <- spp_impacts %>%
  ...
  mutate(eq_wt  = impact_tot / range_tot_km2,
         npp_wt = impact_npp_tot / npp_range_tot_km2) %>%
...
spp_no_cc <- spp_impacts %>%
  ...
  select(sciname, impact_tot, impact_npp_tot, range_tot_km2, npp_range_tot_km2) %>%
  distinct() %>%
  mutate(imp_unwt_tot = impact_tot / range_tot_km2,
         imp_nppwt_tot = impact_npp_tot / npp_range_tot_km2) %>%
  group_by(sciname) %>%
...
iucn_impacts_df <- read_csv(spp_chi_file,
  ...
  group_by(sciname, iucn_sid, vuln, stressor) %>%
...
spp_chi_iucn_clean <- iucn_impacts_df %>%
  ...
  mutate(chi_rel_tot = impact_tot / range_tot_km2,
         ...
         chi_npp_tot = impact_npp_tot / npp_range_tot_km2,
         chi_npp_cst = impact_npp_cst / npp_range_cst_km2,
         chi_ln_npp_tot = impact_ln_npp_tot / ln_npp_range_tot_km2,
         chi_ln_npp_cst = impact_ln_npp_cst / ln_npp_range_cst_km2) %>%
...
mdl1_ln <- glm(threatened ~ chi_ln_npp_cst + impact_ln_npp_cv_cst + log_r_cst, 
           data = df_for_blr_chi %>% filter(qtile_r <= 4),
           family = 'binomial')
summary(mdl1_ln)
with(summary(mdl1_ln), 1 - deviance/null.deviance) ### McFadden's R^2
```
...
mdl2_ln <- glm(threatened ~ chi_ln_npp_tot + impact_ln_npp_cv_tot + log_r_tot, 
           data = df_for_blr_chi %>% filter(qtile_r <= 4),
           family = 'binomial')
summary(mdl2_ln) 
with(summary(mdl2_ln), 1 - deviance/null.deviance)
...
mdl2a_ln <- glm(threatened ~ chi_ln_npp_tot + impact_ln_npp_cv_tot + log_r_tot, 
           data = df_for_blr_chi %>% filter(qtile_r <= 5),
           family = 'binomial')
summary(mdl2a_ln) 
with(summary(mdl2a_ln), 1 - deviance/null.deviance)
...
mdl3_ln <- glm(threatened ~ chi_ln_npp_tot + log_r_tot, 
           data = df_for_blr_chi %>% 
             filter(qtile_r <= 4),
           family = 'binomial')
summary(mdl3_ln) 
with(summary(mdl3_ln), 1 - deviance/null.deviance)
