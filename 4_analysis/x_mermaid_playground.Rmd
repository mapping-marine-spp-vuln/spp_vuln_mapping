...
public_prj <- all_prj %>%
  gather(policy, status, starts_with('data_policy')) %>%
  filter(status != 'Private')
pub_prj_id <- public_prj$id %>% unique()

pub_prj_data <- mermaid_get_project_data(project = pub_prj_id, method = 'all', data = 'sampleevents')
```
