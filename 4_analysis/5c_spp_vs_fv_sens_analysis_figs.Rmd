...
for(s in str_vec) {
  # s <- str_vec[1]
  impact_fig_f <- sprintf('_figs/impact_%s_diff_by_species_map.png', s)
  ...
  if(!file.exists(impact_fig_f)) {
    ...
    impact_df <- fread(impact_fs[s]) %>%
      ...
      mutate(diff = impact_mean_spp - impact_mean_fe) %>%
      left_join(cell_xy, by = 'cell_id')
...
ggsave(plot = p, filename = '_figs/summed_sig_higher_stressors.png', width = 6.5, height = 3, dpi = 300)

knitr::include_graphics('_figs/summed_sig_higher_stressors.png')
...
ggsave(plot = p, filename = '_figs/summed_sig_lower_stressors.png', width = 6.5, height = 3, dpi = 300)

knitr::include_graphics('_figs/summed_sig_lower_stressors.png')
