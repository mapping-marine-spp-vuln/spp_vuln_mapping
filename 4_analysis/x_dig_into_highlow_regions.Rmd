...
* High hab, low FE across all, CC, and non-CC
    * Province 5: Cold temperate NW Atlantic
    * Province 6: Warm temperate NW Atlantic
* High hab, low FE across all, CC, BUT low hab/high FE for non-climate!
    * Province 37: Hawaii
* Low hab, high FE across all, CC, BUT high hab/low FE for non-climate
    * Procince 22: Central Indian Ocean Islands
* Similar hab, FE across all and CC, BUT low hab/high FE for non-climate
    * Province 53: North New Zealand
    * Province 54: South New Zealand
    * Province 62: Subantarctic NZ
    
### Identify cell IDs for these subregions

```{r}
prov_vec <- c(5, 6, 37, 22, 53, 54, 62)
...
diff_df <- diff_df_all %>%
  ...
  group_by(prov_code) %>%
  mutate(cc_q = ntile(abs(cc_diff), n_q),
         noncc_q = ntile(abs(noncc_diff), n_q)) %>%
  ungroup()


diff_top10 <- diff_df %>%
  filter(cc_q == n_q & noncc_q == n_q) %>%
  select(cell_id,
         fe_cc_pct, fe_noncc_pct, hab_cc_pct, hab_noncc_pct,
         cc_diff, noncc_diff, prov_code)

diff_top10$prov_code %>% table()
...
stressor_f_df <- data.frame(f = list.files(here('_data/stressors_mol'), full.names = TRUE)) %>%
  ...
  inner_join(read_csv(here('_raw/stressor_vulnerability_lookup.csv')), by = c('stressor_match' = 'stressor')) %>%
...
bind_maps_list <- function(maps_list, spp_for_calc, join_col = 'map_f') {
...
prov_spp_df <- bind_maps_list(prov_spp_list, spp_map_fs, join_col = 'map_f') %>%
...
spp_sst_rise_maps <- collect_sst_rise_maps(sst_mapfile_df, sst_rise_cells = cell_vec) %>%
  left_join(spp_sst_vuln_df, by = 'species') %>%
  select(species, cell_id, vulnerability, v_score, intensity = therm_prs) %>%
  group_by_at(vars(-intensity)) %>%
  summarize(intensity = mean(intensity, na.rm = TRUE), .groups = 'drop') %>%
  mutate(impact = ifelse(!is.na(intensity), v_score * intensity, 0))
...
* High hab, low FE across all & CC (diff < 0), but low hab/high FE for non-CC (diff > 0)
    * Province 37: Hawaii
...
Most high non-cc impacts are due to microplastics (Pacific Gyre, but plastics not in hab approach).
