...
For each cell, across all iterations, we calculate a difference between the FE approach and spp approach, and then identify the 97.5% and 2.5% values across all iterations to calculate a confidence interval.  Note that for the difference calculation, a positive value indicates that the FE approach estimated a HIGHER impact than the SPP approach.
...
chi_orig_df <- c(chi_orig_spp_r, chi_orig_fe_r, cell_id_r) %>%
  as.data.frame() %>%
  filter(cell_id %in% str_cell_df$cell_id)
...
calc_iter_chi <- function(i, tmp_df) {
  ### i <- 1
  message('Calculating mean chi for iteration ', i, '...')
  iter_df <- tmp_df %>%
    filter(iter == i)
...
if(!file.exists(chi_out_f)) {
  ...
  cell_chi_df <- lapply(1:n_sims, 
                        ...
                        tmp_df = tmp_df) %>%
    rbindlist() %>%
    oharac::dt_join(chi_orig_df, type = 'left', by = 'cell_id') %>%
    mutate(fe_spp_diff  = chi_fe - chi_spp,
           spp_spp_diff = chi_spp - chi_orig_spp,
           fe_fe_diff   = chi_fe - chi_orig_fe)
  ...
  cell_chi_summary <- cell_chi_df %>%
    ...
    .[ , .(chi_mean_spp = mean(chi_spp),
           ...
           diff_95lo = quantile(fe_spp_diff, .025),
           diff_95hi = quantile(fe_spp_diff, .975),
           diff_95lo_spp = quantile(spp_spp_diff, .025),
           diff_95hi_spp = quantile(spp_spp_diff, .975),
           diff_95lo_fe  = quantile(fe_fe_diff, .025),
           diff_95hi_fe  = quantile(fe_fe_diff, .975),
...
noncc_orig_spp_r <- rast(here('_output/cumulative_impact_maps',
                              c('species_group_chi_fishing.tif', 
                                'species_group_chi_land_based.tif',
                                'species_group_chi_ocean_based.tif'))) %>%
  sum() %>%
  setNames('chi_orig_spp')
noncc_orig_fe_r <- rast(here('_output/cumulative_impact_maps',
                              c('funct_entity_group_chi_fishing.tif', 
                                'funct_entity_group_chi_land_based.tif',
                                'funct_entity_group_chi_ocean_based.tif'))) %>%
  sum() %>%
  setNames('chi_orig_fe')

noncc_orig_df <- c(noncc_orig_spp_r, noncc_orig_fe_r, cell_id_r) %>%
  as.data.frame() %>%
  filter(cell_id %in% str_cell_df$cell_id)

if(!file.exists(chi_noncc_out_f)) {
  ### unlink(chi_noncc_out_f)
  cell_chi_df <- lapply(1:n_sims, 
                        FUN = calc_iter_chi, 
                        tmp_df = noncc_tmp_df) %>%
    rbindlist() %>%
    oharac::dt_join(noncc_orig_df, type = 'left', by = 'cell_id') %>%
    mutate(fe_spp_diff  = chi_fe - chi_spp,
           spp_spp_diff = chi_spp - chi_orig_spp,
           fe_fe_diff   = chi_fe - chi_orig_fe)
...
cc_orig_spp_r <- rast(here('_output/cumulative_impact_maps',
                           'species_group_chi_climate.tif')) %>%
  sum() %>%
  setNames('chi_orig_spp')
cc_orig_fe_r <- rast(here('_output/cumulative_impact_maps',
                          'funct_entity_group_chi_climate.tif')) %>%
  sum() %>%
  setNames('chi_orig_fe')
...
### unlink_fs <- list.files(dirname(impact_out_fstem), pattern = 'species.csv', full.names = TRUE)
### unlink(unlink_fs)
...
for(s in strs) {
  ...
  if(!file.exists(impact_out_f)) {
    cell_imp_df <- lapply(1:n_sims, 
      ...
      mutate(fe_spp_diff = impact_fe - impact_spp)
