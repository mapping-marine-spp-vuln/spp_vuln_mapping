...
It appears that a k between 6-12 is reasonable.  Let's choose 11 as suggested by the silhouette method.  Identify clusters across all species and then examine cluster breakdown by class.  Compare `pam()` and `kmeans()`.
...
``` {r}
k_vs_p <- data.frame(k = k15$cluster,
                     p = p15$cluster) %>%
  group_by(k, p) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  ...
  mutate(k_fct = fct_inorder(as.character(k)),
         ...
         k_ltr = LETTERS[as.integer(k_fct)],
         p_ltr = LETTERS[as.integer(p_fct)])
ggplot(k_vs_p, aes(x = k_ltr, y = p_ltr)) +
  geom_jitter()
...
df_clusters <- df_clean %>%
  mutate(k_clust = k15$cluster,
         p_clust = p15$cluster) %>%
  left_join(k_vs_p %>% select(k, p, k_ltr, p_ltr) %>% distinct(), 
            by = c('k_clust' = 'k', 'p_clust' = 'p'))

write_csv(df_clusters %>%
            select(species, where(is.numeric), 
                   k_group = k_ltr, p_group = p_ltr, 
