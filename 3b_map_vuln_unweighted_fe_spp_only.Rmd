...
Apply vulnerability scores to mapped species.  Map out vulnerability to each stressor, first unweighted by species (each spp counts same, this script), then weighted by functional vulnerability (FEs with higher FV are given greater weight, script 3).
...
out_stem <- here_anx('vuln_maps/vuln_unweighted/vuln_by_str_tx/vuln_tx_%s_%s_%s.tif')
...
#   }
#   if(flag) {
#     message('    ... pooled variance for ', length(mean_vec), ' observations...')
#   }
#   var_df <- data.frame(m = mean_vec, s = sdev_vec, n = n_vec)
#   iter_ct <- 1
#   while(nrow(var_df) > 1) {
#     if(flag) {
#       message('  ... processing iteration ', iter_ct, '...')
#     }
#     iter_ct <- iter_ct + 1
#     ### divide vectors into two-element chunks
#     n_chunks <- ceiling(nrow(var_df) / 2)
#     var_df <- var_df %>%
#       mutate(i = rep(1:n_chunks, length.out = n()))
#     
#     var_list <- lapply(
#       1:n_chunks,
#       FUN = function(j) {
...
#           filter(i == j)
#         
#         if(nrow(df_j) == 1) return(df_j %>% select(-i))
#         
#         m_x <- df_j$m[1]; m_y <- df_j$m[2]
#         n_x <- df_j$n[1]; n_y <- df_j$n[2]
#         s_x <- df_j$s[1]; s_y <- df_j$s[2]
#         var_j <- pooled_var(x_bar = m_x, y_bar = m_y, 
#                             s_x   = s_x, s_y   = s_y,
#                             n_x   = n_x, n_y   = n_y)
#     
#         ### set up values for next iteration
#         df_out <- data.frame(m = (m_x * n_x + m_y * n_y) / (n_x + n_y),
#                              s = sqrt(var_j),
#                              n = n_x + n_y)
#     })
#     var_df <- bind_rows(var_list)
#   }
#   
#   var_out <- (var_df$s)^2
#   return(var_out)
...
# 
# var_out <- iterated_pooled_var_old(mean_vec, sdev_vec, n_vec)
# 
...
out_stem <- here('_output/vuln_maps_unweighted/vuln_all_spp_%s_%s.tif')
...
for(stressor in strs) {
  ...
  vuln_nspp_rast_f <- here('_output/nspp_maps/nspp_in_unweighted_vuln_maps.tif')
  if(!file.exists(vuln_nspp_rast_f)) {
    ...
    message('Creating species richness raster for unweighted vulnerability maps...')
    rast_nspp_vuln <- map_to_mol(vuln_map_df, by = 'cell_id', which = 'n_spp')
    writeRaster(rast_nspp_vuln, vuln_nspp_rast_f, overwrite = TRUE)
  }
...
vuln_all_spp_dir <- here('_output/vuln_maps_unweighted')
