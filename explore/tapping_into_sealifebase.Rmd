---
title: "Explore SeaLifeBase interface and data"
...
library(rfishbase)
# Sys.setenv(FISHBASE_API="sealifebase") ### this doesn't seem to work
```

# Summary

This script is identical to tapping_into_fishbase, just applied to the sealifebase database.
...
```{r get species table}
spp_info <- species(server = 'sealifebase')
...
fields <- c('ana_cat', 'land', 'depth', 'length', 'weight', 'longevity', 'demers', 'pd50')
...
```{r food table}
food <- fooditems(server = 'sealifebase')
...
### Diet:
``` {r diet table}
food_diet <- diet(server = 'sealifebase')
...
troph_ecol <- ecology(server = 'sealifebase')
...
reprod <- reproduction(server = 'sealifebase')
...
``` {r maturity table}
matur  <- maturity(server = 'sealifebase')
...
### spawning
```{r spawning table}
spawn  <- spawning(server = 'sealifebase')
...
```{r fecundity table}
fecund <- fecundity(server = 'sealifebase')
...
``` {r ecosystem table}
ecosys <- ecosystem(server = 'sealifebase')
...
* [The POPCHAR Table](https://www.fishbase.se/manual/English/fishbasethe_popchar_table.htm) 
    ...
    * `popchar(server = 'sealifebase')` - local population values, rather than global
...
* [The LENGTH-FREQUENCY Table](https://www.fishbase.se/manual/English/lengthfrequency.htm)
    ...
    * `length_freq(server = 'sealifebase')`
* [The LENGTH-LENGTH Table](https://www.fishbase.se/manual/english/PDF/FB_Book_CBinohlan_Length-Length_RF_JG.pdf)
    ...
    * `length_length(server = 'sealifebase')`
...
lw <- length_weight(server = 'sealifebase')
...
``` {r pop growth table}
popg <- popgrowth(server = 'sealifebase')
...
``` {r larvae table}
larv <- larvae(server = 'sealifebase')
...
### morphology
```{r morphology}
morph <- morphology(server = 'sealifebase')
...
### brains
```{r brains}
brain <- brains(server = 'sealifebase')
...
```{r oxygen}
oxyg <- oxygen(server = 'sealifebase')
...
```{r swimming}
swim <- swimming(server = 'sealifebase')
