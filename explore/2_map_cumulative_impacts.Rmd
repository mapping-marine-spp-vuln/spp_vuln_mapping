...
plot(mean_chi_rast,  main = 'mean CHI',         col = hcl.colors(n = 50))
plot(sd_chi_rast,    main = 'sd CHI',           col = hcl.colors(n = 50))
plot(mean_nstr_rast, main = 'mean # stressors', col = hcl.colors(n = 50))
plot(nspp_rast,      main = '# spp',            col = hcl.colors(n = 50))
...
```{r}
cv_rast <- sd_chi_rast / mean_chi_rast
cv_df <- rasterToPoints(cv_rast) %>%
  as.data.frame()
ggplot() +
  ggtheme_map() +
  geom_raster(data = cv_df, aes(x, y, fill = layer)) +
  scale_fill_viridis_c(option = 'viridis') +
  coord_sf() +
  labs(fill = 'CV', title = 'Coefficient of variation')
```
...
x <- ggplot(plot_df %>% filter(rank == 'high')) +
  ...
  facet_wrap(~cell_id, ncol = 5) +
  labs(x = 'n_spp')

ggsave('high_cells_dists.png', height = 3, width = 10, dpi = 300)
...
ggsave('low_cells_dists.png', height = 3, width = 10, dpi = 300)
...
```{r plot }
maps <- read_csv(here('_data/spp_marine_maps_2020-1.csv'))
spp_ids <- c(8005, 39397, 13006)
spp_names <- maps %>%
  filter(iucn_sid %in% spp_ids) %>%
  arrange(iucn_sid) %>%
  select(iucn_sid, sciname)

spp_fstem = sprintf('spp_str_intsx_%s_', spp_ids) %>% paste0(collapse = '|')
spp_fs <- list.files(dir_intsx, pattern = spp_fstem, full.names = TRUE)

spp_df <- lapply(spp_fs, read_csv, col_types = 'id') %>%
  setNames(basename(spp_fs)) %>%
  bind_rows(.id = 'f') %>%
  mutate(iucn_sid = str_extract(f, '[0-9]+') %>% as.integer()) %>%
  group_by(iucn_sid, cell_id) %>%
  summarize(chi = sum(impact))

for(spp_id in spp_ids) { # spp_id <- spp_ids[1]
  s_df <- spp_df %>%
    filter(iucn_sid == spp_id)
  nm <- spp_names %>% filter(iucn_sid == spp_id) %>% .$sciname
  r <- subs(cell_id_rast, s_df, by = 'cell_id', which = 'chi')
  plot_map(r, t = nm, f = 'CHI')
}
```
