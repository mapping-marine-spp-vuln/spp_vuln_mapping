...
Filter to just SST rise stressor.
...
out_stem <- here_anx('impact_maps_by_taxon/impacts_tx_sst_rise', 
                     'imp_unweighted_sst_rise_all',
                     'imp_unweighted_sst_rise_%s_%s.tif')
### zxcv <- list.files(dirname(out_stem), full.names = TRUE)
### unlink(zxcv)
for(t in taxa) {
  ### t <- taxa[10]
  tx_vuln_df <- spp_for_vuln_calc %>%
    filter(taxon == t) %>%
  ...
  message('Processing mean/sd vulnerability by species in taxon ', t, ' to thermal stressor...')
  ...
  result_list <- parallel::mclapply(1:n_chunks, mc.cores = n_cores,
                 FUN = function(n) { ### n <- 3
                   ...
                   chunk_sum <- tx_maps %>%
                     ...
                     mutate(impact = score * therm_prs) %>%
                     group_by(cell_id) %>%
                     summarize(impact_mean = mean(impact) %>% round(5),
                               impact_sd   = sd(impact) %>% round(5),
...
impact_map_stem <- here('_output/impact_maps/impact_maps_unweighted_all', 
                        'impact_unweighted_sst_rise_%s.tif')
...
if(any(!file.exists(impact_f_mean, impact_f_mean))) {
...
plot(log10(mean_rast_unwt), col = map_cols, 
     main = 'log_10(Mean) unweighted impact: SST rise (all spp)',
     legend = TRUE, axes = FALSE)  
plot(sdev_rast_unwt, col = map_cols, 
     main = 'CV unweighted weighted impact: SST rise (all spp)',
     legend = TRUE, axes = FALSE)
```
...
spp_for_vuln_fe_only <- spp_for_vuln_calc %>%
  inner_join(spp_fe, by = 'species')
  
taxa <- spp_for_vuln_fe_only$taxon %>% unique() %>% sort()

out_stem_fe <- here_anx('impact_maps_by_taxon/impacts_tx_sst_rise', 
                        'imp_unweighted_sst_rise_fe_only',
                        'imp_unweighted_sst_rise_fe_%s_%s.tif')
### zxcv <- list.files(dirname(out_stem_fe), full.names = TRUE)
### unlink(zxcv)
for(t in taxa) {
  ### t <- taxa[10]
  tx_vuln_df <- spp_for_vuln_fe_only %>%
    filter(taxon == t) %>%
    select(species, score) %>%
    distinct()
  ...
  message('Binding thermal stress maps for taxon ', t, ' FE species...')
  tx_maps <- tx_maps_list %>%
    setNames(tx_mapfiles$species) %>%
    data.table::rbindlist(idcol = 'species')
  ...
  message('Processing mean/sd vulnerability by FE species in taxon ', t, ' to thermal stressor...')
...
impact_map_stem_fe <- here('_output/impact_maps/impact_maps_unweighted_fe_only', 
                        'impact_unweighted_fe_only_sst_rise_%s.tif')
  ### %s is parameter (mean, sd, nspp)
...
if(any(!file.exists(impact_f_mean, impact_f_mean))) {
  ### Combine mean, sdev, and nspp maps by taxon into one big dataframe
  message('Processing mean, sd, nspp maps across FE species for sst rise stressor...')
  big_df <- combine_taxa_maps(fe_tx_sst_map_df)
}
...
plot(log10(mean_rast_unwt), col = map_cols, 
     main = 'log_10(Mean) unweighted impact: SST rise (FE spp only)',
     legend = TRUE, axes = FALSE)  
plot(sdev_rast_unwt, col = map_cols, 
     main = 'CV unweighted weighted impact: SST rise (FE spp only)',
     legend = TRUE, axes = FALSE)
```
...
```{r helper functions}
read_truncated_temp_str_map <- function(f, chunk_start, chunk_end) {
  if(file.exists(f)) {
    df <- data.table::fread(f) %>%
...
bind_maps_list <- function(chunk_maps_list, spp_for_vuln_fe_only) {
  ...
  if(nrow(chunk_maps_raw) > 0) {
    ...
    chunk_maps_raw <- chunk_maps_raw %>%
      oharac::dt_join(spp_for_vuln_fe_only, by = 'map_f', type = 'left') %>%
      select(-map_f, -src) %>%
      distinct()
  }
...
calc_chunk_str_sum <- function(chunk_maps, spp_for_vuln_fe_only) {
  chunk_spp_vuln <- spp_for_vuln_fe_only %>%
...
tmp_stem_fv <- here_anx('impact_maps_by_taxon/impacts_tx_sst_rise', 
                        'imp_fv_weighted_sst_rise',
                        'impact_summary_chunk_%s_to_%s_sst_rise.csv')
### zxcv <- list.files(dirname(tmp_stem_fv), full.names = TRUE)
### unlink(zxcv)

mapfile_fe_df <- mapfile_df %>%
  filter(species %in% spp_for_vuln_fe_only$species) %>%
  distinct()
...
for(chunk_i in 1:n_chunks) { 
  ...
  tmp_sst_rise_fv <- sprintf(tmp_stem_fv, chunk_start, chunk_end)
  if(file.exists(tmp_sst_rise_fv)) {
  ...
  message('Loading ', nrow(mapfile_fe_df), ' rangemaps cropped for ', chunk_text,  '...')
  
  chunk_maps_list <- parallel::mclapply(mapfile_fe_df$map_f, mc.cores = 30, 
                                        ### f <- mapfile_fe_df$map_f[1]
                                        FUN = read_truncated_temp_str_map, 
                                        chunk_start = chunk_start, chunk_end = chunk_end) 
  
  chunk_maps_raw <- bind_maps_list(chunk_maps_list, mapfile_fe_df)
  ...
  if(nrow(chunk_maps_raw) == 0) {
    ...
    write_csv(data.frame(), tmp_sst_rise_fv)
    next()
  }
  ...
  chunk_str_sum <- calc_chunk_str_sum(chunk_maps, spp_for_vuln_fe_only)
  
  write_csv(chunk_str_sum, tmp_sst_rise_fv)
}
```
...
rast_fv_out_stem <- here('_output/impact_maps/impact_maps_fv_weighted', 
                         'impact_fv_weighted_sst_rise_%s.tif')
...
if(any(!file.exists(rast_mean_impact_fv_f, rast_sdev_impact_fv_f))) {
  ...
  impact_map_df <- parallel::mclapply(chunk_files, mc.cores = 33, 
    ...
    bind_rows() %>%
    mutate(fv_wt_mean_impact = round(fv_wt_mean_impact, 5),
           fv_wt_sd_impact   = round(fv_wt_sd_impact,   5))
  
  message('Converting impact maps to rasters for sst rise...')
  mean_rast_fvwt <- map_to_mol(impact_map_df, 
                               by = 'cell_id', which = 'fv_wt_mean_impact')
...
plot(log10(mean_rast_fvwt), col = map_cols, 
     main = 'log_10(Mean) FV-weighted impact: SST rise',
     legend = TRUE, axes = FALSE)  
plot(sdev_rast_fvwt, col = map_cols, 
     main = 'CV FV-weighted weighted impact: SST rise',
     legend = TRUE, axes = FALSE)
```
...
plot(mean_diff_squished$r, col = map_cols, 
     main = '% Diff in mean vuln: SST rise...',
     zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
     legend = TRUE, axes = FALSE)  
plot(m_d_mask_0.10, col = map_cols, 
     main = '% Diff in mean vuln masked 0.10: SST rise...',
     zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
     legend = TRUE, axes = FALSE)  
plot(m_d_mask_0.25, col = map_cols, 
     main = '% Diff in mean vuln masked 0.25: SST rise...',
     zlim = c(-mean_diff_squished$zlim, mean_diff_squished$zlim), 
     legend = TRUE, axes = FALSE)
```
