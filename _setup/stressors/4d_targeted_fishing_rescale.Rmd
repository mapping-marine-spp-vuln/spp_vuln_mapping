...
spp_vec <- watson_am_lookup$spp %>% unique() %>% sort()

npp_catch_fstem <- here_anx('stressors/fishing/npp_norm_catch_by_spp_moll',
                            'am_spp_npp_catch_%s_mol.csv')
range_map_fstem <- here_anx('spp_maps_mol', 'am_spp_mol_%s.csv')
```

``` {r calc ref point by range}
calc_quantiles_range <- function(s, fstem) {
  # s <- spp_vec[1]
  s_snake <- str_replace_all(s, '[^a-z]+', '_')
  i <- which(s == spp_vec)

  out_f <- sprintf(fstem, s_snake)

  if(!file.exists(out_f)) {
    message('Processing ', s, '... (', i, ' of ', length(spp_vec), ')')
    catch_df <- read_csv(sprintf(npp_catch_fstem, s_snake), 
                         show_col_types = FALSE)
    
    range_df <- read_csv(sprintf(range_map_fstem, s_snake), 
                         show_col_types = FALSE)
...
if(!file.exists(fishing_ref_f)) {
  ref_tmp_fstem <- here('tmp', 'fishing_str_ref_%s.csv')

  tmp <- parallel::mclapply(spp_vec, mc.cores = 16,
...
ref_pt <- max(qtile_results$x99_9_percent)
```
...
rescaled_fstem <- here_anx('stressors/fishing/rescaled_catch_by_spp_cell',
                           'am_spp_rescaled_catch_%s.csv')
rescale_catch <- function(s, ref_pt) {
  # s <- missing_spp[2]
  # s <- 'pleoticus muelleri'
  # s <- 'abyssaster planus'
  s_snake <- str_replace_all(s, '[^a-z]+', '_')
  i <- which(s == spp_vec)
  
  out_f <- sprintf(rescaled_fstem, s_snake)
  if(!file.exists(out_f)) {
    message('Rescaling catch for ', s, '... (', i, ' of ', length(spp_vec), ')')
...
tmp <- parallel::mclapply(spp_vec, mc.cores = 24,
