...
Gather stressor layers and process to the CRS for this analysis: Mollweide, 10 km x 10 km.  Identify gapfill needs if necessary.  Note in methodology: where in CHI at ~1 km Mollweide we frequently use the 99.99%ile as a reference point, here I am using the 99.9%ile (one fewer decimal).  Since we've aggregated by a factor of 100 (10 x 10), it seems like 99.9% will still clip out the very most extreme values.
...
```{r GFW data for benthic disturbance, eval = FALSE}
...
```{r Nutrient plume data, eval = FALSE}
...
```{r Sea level rise data, eval = FALSE}
...
```{r Ocean acidification data, eval = FALSE}
...
```{r Ultraviolet radiation data, eval = FALSE}
...
See separate script that pre-processes these, masking to ocean cells and aggregating by mean to 0.1° resolution.
...
```{r}
dir_shipping <- '/home/shares/ohi/spp_vuln/spp_vuln_mapping/stressors/ship_density/aggregated'
...
```{r create large ship layer, eval = FALSE}
...
if(!file.exists(ship_large_f)) {
  ...
  ship_large_stack <- projectRaster(orig_stack, ocean_rast, progress = 'text')
...
if(!file.exists(ship_small_f)) {
  ...
  ### Already masked and aggregated in prep script
  ship_small_stack <- projectRaster(orig_stack, ocean_rast, progress = 'text')
  ### projectRaster with ngb introduces some weird negative values; drop'em
...
# 
#   ### Already masked and aggregated in prep script
#   ship_all_stack <- projectRaster(orig_stack, ocean_rast, progress = 'text')
#   ### projectRaster with ngb introduces some weird negative values; drop'em
...
benth_f <- here_ohi('_data/stressors_mol/benth_str_2020.tif')
### unlink(benth_f)
...
if(!file.exists(benth_f)) {
  ...
  ocean_mask_sf <- read_sf(here('_spatial/ne_10m_ocean/ne_10m_ocean.shp'))
  benth_r <- raster::extend(benth_r, ocean_mask_sf)
  values(benth_r)[is.na(values(benth_r))] <- 0

  ocean_mask_r <- fasterize::fasterize(ocean_mask_sf, benth_r)
  masked_r <- mask(benth_r, ocean_mask_r, progress = 'text')
  ...
  ### round to drop ludicrous precision then save out
  values(ship_small_r) <- round(values(ship_small_r), 5)
  writeRaster(ship_small_r, benth_f, overwrite = TRUE)
}
