...
iucn_spp_info <- read_csv(here('_data/iucn_spp', 'iucn_marine_spp_info_2021-3.csv'))
...
am_T_env <- get_am_spp_envelopes() %>%
  ...
  left_join(iucn_spp_info %>% select(iucn_sid, sciname), by = 'sciname')

iucn_T_env <- read_csv(here('_setup/stressors/int/iucn_spp_thermal_env.csv')) %>%
  left_join(iucn_spp_info, by = 'iucn_sid') %>%
  filter(!sciname %in% am_T_env$sciname) %>%
...
# unlink(list.files(dirname(out_fstem), full.names = TRUE))
...
for(i in 1:nrow(gps_df)) {
  ...
  tmp <- parallel::mclapply(1:nrow(spp_envs), mc.cores = 16, 
    ...
    # for(j in 1:nrow(spp_envs)) { 
      ...
      ### to check results:
      # spp_range <- map_to_mol(spp_map, by = 'cell_id', which = 'cell_id'); plot(spp_range, col = 'black')
      # spp_str <- map_to_mol(spp_temp_df, by = 'cell_id', which = 'therm_prs'); plot(spp_str)
...
for(i in 1:nrow(spp_envs)) {
  ...
  p <- ggplot() +
    ...
    theme_void() +
    labs(fill = 'Pressure',
         title = sprintf('%s: T_max_p = %s°C, T_max_a = %s°C', spp, tmax_p, tmax_a))
