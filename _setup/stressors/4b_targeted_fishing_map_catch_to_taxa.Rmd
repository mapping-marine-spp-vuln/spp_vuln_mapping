...
process_species_catch <- function(s, spp_vec, spp_map_fstem, source = 'aquamaps') {
  ...
  if(!file.exists(spp_map_f)) {
    ...
    spp_map <- map_tx_spp_rich(spp_ids, 
                               am_spp_cells = am_spp_cells, iucn_spp_cells = iucn_spp_cells) %>%
      select(-n_spp)
...
am_spp_map_fstem <- here_anx('stressors/fishing/total_catch_by_spp_cell',
                          'am_spp_catch_%s.csv')
...
iucn_spp_map_fstem <- here_anx('stressors/fishing/total_catch_by_spp_cell',
                          'iucn_spp_catch_%s.csv')

x <- list.files(dirname(iucn_spp_map_fstem), pattern = 'iucn_spp_catch', full.names = TRUE)
### unlink(x)
