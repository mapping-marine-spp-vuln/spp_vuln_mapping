...
am_spp_ids <- get_am_spp_info() %>%
  select(am_sid, sciname, occur_cells) %>%
  distinct()

am_spp_valid <- am_spp_ids %>%
...
# watson_maps_spp_df$spp %>% n_distinct() 
    # 26169 distinct species across AquaMaps and IUCN
# watson_maps_spp_df %>% filter(am_sid %in% am_spp_valid$am_sid | !is.na(iucn_sid)) %>% .$spp %>% n_distinct()
#   # 20326 species when accounting for occur_cells >= 10; 2000+ of these are in IUCN
...
w_gear_df <- readxl::read_excel(codes_xlsx, sheet = 'Gear') %>%
  janitor::clean_names() %>%
  select(-x9, -x10)
...
iucn_spp_cells <- read_csv(here_anx('iucn_spp', 'iucn_spp_hcaf.csv'))
```
...
```{r drop big species from certain taxa}
...
watson_maps_lookup <- watson_am_cleaned_df %>%
  select(taxonkey, am_sid, iucn_sid, spp) %>%
  distinct()
...
map_tx_catch <- function(tx_id, catch_df) {
  ...
  ### at this point, not using industrial vs non-industrial...
...
process_higher_taxa <- function(i) {
  ### i <- 1
...
process_species_catch <- function(s, spp_map_fstem) {
...
spp_vec <- watson_am_cleaned_df$spp %>% 
  unique() %>% sort()
    ### 23801 spp

spp_map_fstem <- here_anx('stressors/fishing/total_catch_by_spp_cell',
                          'am_spp_catch_%s.csv')
### unlink(list.files(dirname(spp_map_fstem), full.names = TRUE))

tmp <- parallel::mclapply(spp_vec, mc.cores = 12,
                          FUN = process_species_catch, 
                          spp_map_fstem = spp_map_fstem)

### to run just those spp without files already (e.g., unlinked zero-
### length files and need to just rerun those)
# x <- list.files(dirname(spp_map_fstem), full.names = TRUE)
# 
# spp_with_files <- basename(x) %>%
#   str_remove_all('spp_catch_|.csv') %>%
#   str_replace_all('_', ' ')
# 
# spp_wo_files <- spp_vec[!spp_vec %in% spp_with_files]
# 
# tmp <- parallel::mclapply(spp_wo_files, mc.cores = 12,
#                           FUN = process_species_catch)
