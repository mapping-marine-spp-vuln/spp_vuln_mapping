...
```{r}
big_spp <- read_csv(here('_data/grouping_traits_post_imputation.csv')) %>%
  mutate(len = exp(log_l)) %>%
  filter(len > 30) %>%
  .$species

### if 100039, drop large species
watson_am_spp_df <- watson_am_spp_df %>%
  filter(!(taxonkey == 100039 & spp %in% big_spp))
...
for(i in 1:nrow(tx_high)) {
  ### i <- which(tx_high$taxonkey == 100039)
  ...
  if(!file.exists(tx_map_f)) {
    ...
    if(nrow(tx_map_out) == 0) {
      ...
      tx_map_out <- data.frame(loiczid = -1)
    }
...
spp_vec <- watson_am_spp_df$spp %>% unique() %>% sort()
    ### 24701 spp across 25099 species IDs
