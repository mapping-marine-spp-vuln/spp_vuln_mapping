...
# watson_am_spp_df$spp %>% n_distinct() 
    # 24701 distinct species in AquaMaps
# watson_am_spp_df %>% filter(am_sid %in% am_spp_valid$am_sid) %>% .$spp %>% n_distinct()
#   # 18174 species when accounting for occur_cells >= 10
...
### write out lookup table for Watson taxonkey to AquaMaps species and id
watson_am_lookup <- watson_am_cleaned_df %>%
  select(taxonkey, am_sid, spp) %>%
  distinct()
write_csv(watson_am_lookup, here('_setup/stressors/int/watson_am_lookup.csv'))
...
process_species_catch <- function(s) {
