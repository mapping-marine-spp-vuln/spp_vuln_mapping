...
# catch_fs <- list.files(here_anx('stressors/fishing/2_total_catch_by_spp_cell'), full.names = TRUE)
# 
...
iucn_spp_match <- read_csv(here('_data/iucn_spp/iucn_to_worms_match.csv')) %>%
  filter(mapped == TRUE)
...
process_species_to_moll <- function(f) {
  ### 14 30 46 62 78 94
  # f <- spp_hcaf_fs[14]
  # f <- spp_hcaf_fs[str_detect(spp_hcaf_fs, 'iucn_')][1]
  # f <- "/home/shares/ohi/spp_vuln/spp_vuln_mapping/stressors/fishing/2_total_catch_by_spp_cell/am_spp_catch_acanthomenia_arcuata.csv"
  new_f <- str_replace(basename(f), 'spp_catch', 'spp_npp_catch') %>%
    str_replace('.csv', '_mol.csv')
  spp_mol_map_f <- file.path(spp_mol_map_dir, new_f)
  ...
  if(!file.exists(spp_mol_map_f)) {
    ...
    spp_catch_mol <- spp_catch_hcaf %>%
      ...
      group_by(loiczid) %>%
      mutate(ocean_a_hcaf = sum(ocean_a_mol * 100)) %>%
      ungroup() %>%
      ### distribute total catch of LOICZID and normalize by NPP
      mutate(pel_sum = pel_tot_catch / ocean_a_hcaf * ocean_a_mol * 100,
             ben_sum = ben_tot_catch / ocean_a_hcaf * ocean_a_mol * 100,
             pel_norm = pel_sum / pel_npp,
             ben_norm = ben_sum / ben_npp) %>%
      drop_na()
    if(nrow(spp_catch_mol) == 0) {
...
spp_mol_map_dir  <- here_anx('stressors/fishing/3_npp_norm_catch_by_spp_moll')

spp_hcaf_fs <- list.files(spp_hcaf_map_dir, full.names = TRUE) %>% unique()

### to cut zero-length files
# x <- list.files(spp_mol_map_dir, full.names = TRUE, pattern = 'am_spp_npp_catch_')
# df <- data.frame(f = x, fsize = file.size(x))
# y <- df %>% filter(fsize < 100) %>% .$f
# unlink(y)

### to run only missing files
x <- list.files(spp_mol_map_dir)
y <- x %>% str_replace('spp_npp_catch', 'spp_catch') %>% str_replace('_mol.csv', '.csv')
...
tmp <- parallel::mclapply(spp_hcaf_fs, mc.cores = 32, FUN = process_species_to_moll)
