...
Read in rasters (as csvs) of fisheries catch per species per cell.  Normalize by NPP (surface and benthic); identify a reference point; save as .csvs of cell ID and normalized catch.  Identify a reference point based on highest normalized catch across all species and cells.
...
```{r set up cell_id to loiczid to ocean_a lookup dataframe}
...
hcaf_area_df <- get_hcaf_info() %>%
  dplyr::select(loiczid, cell_area, ocean_area) %>%
  filter(ocean_area > 0)
```
...
process_species_to_moll <- function(s) {
  ...
  if(!file.exists(spp_mol_map_f)) {
    ...
    spp_catch_hcaf <- read_csv(spp_hcaf_map_f, show_col_types = FALSE) %>%
      oharac::dt_join(hcaf_area_df, by = 'loiczid', type = 'left') %>%
      mutate(pel_per_km2 = pel_tot_catch / ocean_area,
             ben_per_km2 = ben_tot_catch / ocean_area)
...
Alternately: What if we instead took each species fishing stressor map, identified the 99.9%ile for that, and then used the highest one across all species?
