...
process_species_to_moll <- function(f) {
  ...
  if(!file.exists(spp_mol_map_f)) {
    ...
    message('Processing NPP-norm spp-total catch map for ', s, 
            ' (', i, ' of ', length(spp_hcaf_fs), ') based on ', toupper(src), ' spp map...')

    ### Read in catch per LOICZID cell; convert catch per 0.5° cell to catch per km^2^
    spp_catch_hcaf <- read_csv(f, show_col_types = FALSE) %>%
...
spp_hcaf_map_dir <- here_anx('stressors/fishing/total_catch_by_spp_cell')
spp_mol_map_dir  <- here_anx('stressors/fishing/npp_norm_catch_by_spp_moll')
...
# missing <- spp_hcaf_fs[!spp_hcaf_fs %in% z]

tmp <- parallel::mclapply(spp_hcaf_fs, mc.cores = 16, FUN = process_species_to_moll)
```
