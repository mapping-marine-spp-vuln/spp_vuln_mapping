...
Log-transform ($\log(x+1)$)then aggregate.  Loop over each masked layer, save out aggregated layer.
...
for(i in seq_along(tifs_masked)) {
  ...
  if(!file.exists(tif_agg)) {
    message('Log-transforming and aggregating ', basename(tif_masked))
    r <- raster(tif_masked)
    r_log <- log(r + 1)
    r_agg <- aggregate(r_log, fact = 20, fun = mean, progress = 'text')
...
for(t in tifs_agg) {
  ...
  r <- raster(t)
  plot(r, axes = F, main = sprintf('log(%s + 1)', r_name))
}
```
