...
log_N <- log10(N_rast)[N_rast > 0]
hist(log_N, main = 'distribution of log_10(N) non-zero values')
...
N_rast_rescale <- rescale_stressor(N_rast, qtile = .999)
...
### explore effect of log transforming then rescaling:
# log_N_rescale <- rescale_stressor(log_plus(N_rast), qtile = .999)
# y <- values(log_N_rescale)[values(log_N_rescale) > 0]
# hist(y, main = 'distribution of logged then rescaled non-zero values')


hist(log10(x), main = 'distribution of rescaled non-zero values (log10 transformed)')
