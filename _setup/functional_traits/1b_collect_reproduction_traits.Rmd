...
age_mat_vuln <- get_vuln_traits() %>%
  ...
  summarize(trait_value = paste0(unique(trait_value), collapse = ';'),
            .groups = 'drop')

age_mat_downfilled <- downfill(age_mat_vuln) %>%
  distinct()

age_mat_expanded <- age_mat_downfilled %>%
  mutate(trait_value = str_split(trait_value, ';')) %>%
  unnest(trait_value) %>%
  select(species, value = trait_value) %>%
  distinct() %>%
  mutate(source = 'vuln')
```
...
age_mat_all <- bind_rows(age_mat_expanded, mat_fb_binned) %>%
...
fecund_downfilled <- downfill(fecund_vuln) %>%
  distinct()

fecund_expanded <- fecund_downfilled %>%
  mutate(trait_value = str_split(trait_value, ';')) %>%
  unnest(trait_value) %>%
  select(species, value = trait_value) %>%
  distinct() %>%
...
fecund_all <- bind_rows(fecund_expanded, fecund_fb_binned) %>%
  mutate(trait = 'fec') %>%
