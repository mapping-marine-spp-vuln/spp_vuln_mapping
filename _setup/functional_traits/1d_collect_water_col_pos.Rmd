...
Gather data on water column position from FishBase and the species vulnerability project.  Gapfill using downfill methodology.
...
depth_vuln_score <- depth_vuln_downfill %>%
  ...
  select(taxon, species, benthic, pelagic, reef) %>%
  distinct()
```
...
record_df <- collect_records(dem_pel_fb_raw, field = 'demers_pelag', file_tag = 'resolve_names_fb_depth') %>%
...
table(depth_fb %>% select(benthic, pelagic, reef))
...
depth_df <- depth_vuln_score %>%
  ...
  filter(bool == TRUE) %>%
  select(species, value, source, taxon) %>%
