...
mat_worms_fb <- spp_from_worms %>%
  left_join(mat_fb, by = 'species') %>%
...
aphia_records_csv <- here('int/resolve_names_fb_maturity.csv')
...
if(!file.exists(aphia_records_csv)) {
  ...
  record_chunk_stem <- 'tmp/resolve_age_mat_fb_chunk_%s_%s.csv'
  ### unlink(list.files(here('tmp'), pattern = 'resolve_age_mat_fb_chunk_', full.names = TRUE))
  ...
  chunk_fs <- list.files(here('tmp'), pattern = 'resolve_age_mat_fb_chunk_', full.name = TRUE)
...
fecund_worms_fb <- spp_from_worms %>%
  left_join(fecund_fb, by = 'species') %>%
  mutate(db = ifelse(is.na(db), 'worms', db))

no_match <- anti_join(fecund_fb, spp_from_worms, by = 'species')
no_match_w_vals <- no_match %>%
  filter(!is.na(max_fecundity)) %>%
  mutate(genus = str_extract(species, '[a-z]+(?= ?)'),
         genus_match = genus %in% spp_from_worms$genus)
...
aphia_records_csv <- here('int/resolve_names_fb_fecundity.csv')
...
if(!file.exists(aphia_records_csv)) {
  ...
  chunk_size <- 25
  n_chunks <- ceiling(length(names_to_resolve) / chunk_size)
  record_chunk_stem <- 'tmp/resolve_fecund_fb_chunk_%s_%s.csv'
  ### unlink(list.files(here('tmp'), pattern = 'resolve_fecund_fb_chunk_', full.names = TRUE))
  ...
  chunk_fs <- list.files(here('tmp'), pattern = 'resolve_fecund_fb_chunk_', full.name = TRUE)
  record_df <- parallel::mclapply(chunk_fs, read_csv) %>%
    bind_rows() %>%
    distinct()
  write_csv(record_df, aphia_records_csv)
...
```{r parental investment from fishbase}
...
```{r parental involvement from vuln}
