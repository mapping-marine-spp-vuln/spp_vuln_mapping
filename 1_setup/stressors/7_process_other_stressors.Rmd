...
slr_f <- here('_data/stressors_mol/sea_level_rise_2019.tif')
### unlink(slr_f)
...
if(!file.exists(slr_f)) {
  ...
  raw_stack <- raster::stack(local_fs)
  
  raw_r <- calc(raw_stack, mean)
  # class      : RasterLayer 
  ...
  ### Project to Mollweide 10km and masking to coastal 1 km cells
  slr_moll_r <- projectRaster(combo, ocean_rast, progress = 'text')
  coastal_r  <- raster(here('_spatial/coastal_1km_area_mol.tif'))

  # slr_coastal_r <- mask(slr_moll_r, coastal_r, progress = 'text')
  slr_coastal_r <- slr_moll_r * coastal_r
  # missing <- coastal_r
...
oa_f <- here('_data/stressors_mol/ocean_acidification_2020.tif')
### unlink(oa_f)
...
uv_f <- here('_data/stressors_mol/uv_radiation_2020.tif')
### unlink(uv_f)
