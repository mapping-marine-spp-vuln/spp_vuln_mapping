...
### note: vertical migrator gets ignored here; we're looking at horizontal range
...
mob_vuln_downfill <- mob_vuln %>%
  downfill() 

mob_vuln_expand <- mob_vuln_downfill %>%
...
mob_vuln_gf <- mob_vuln_expand %>%
  ...
  mutate(mob_cat = levels(mob_vuln_expand$mob_cat)[value],
         value   = factor(mob_cat, levels = levels(mob_vuln_expand$mob_cat))) %>%
  filter(!is.na(value))
```
