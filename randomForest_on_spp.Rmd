...
train <- traits_dropna[-holdout, ]
```

Because we have nine predictor variables, select `mtry = 3` at a time ($\sqrt{\text{# of traits}}$).  Note randomForest seems to scale ~ $n^{1.9}$ (on my laptop - may be different on Mazu?)
...
Supervised RF does a great job of predicting a species' class based on the traits given.  What will happen when we run an unsupervised RF to inform a clustering algorithm?
...
Iterate over different values of $k$ and identify a "knee".  Since we will be using Partitioning Around Medioids method, look for a knee using that method.
``` {r knee method}
set.seed(42)
wss <- function(k, mtx) {
...
dist_mtx <- as.dist(1 - unsup_rf$proximity)
dist_mtx <- dist_mtx[1:1000, 1:1000]
# Compute and plot wss for a range of k
k_vec <- 1:20
wss_values <- sapply(k_vec, wss, mtx = dist_mtx1)
...
Now the unsupervised random forest model is fit we’ll extract the proximity matrix and use this as input to a PAM procedure.  Based on prior analysis, let's try group into k = 15.  
