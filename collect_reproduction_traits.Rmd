...
gen_time_gf <- gen_time_df %>%
  gapfill_up_down(col = 'log_gen_time', keep = keep_gf_levels)
```
...
max_fecund_gf <- max_fecund_df %>%
  gapfill_up_down(col = 'log_max_fecund', keep = keep_gf_levels)
```
...
parental_protect_gf <- parental_protect_df %>%
  gapfill_up_down(col = 'protect_score', keep = keep_gf_levels)
...
parental_depend_gf <- depend_vuln_gf %>%
  ...
  gapfill_up_down(col = 'depend_score', keep = keep_gf_levels)
...
pld_vuln_gf <- pld_vuln_downfill %>%
  full_join(protect_df %>% select(-protect_score)) %>%
  gapfill_up_down(col = 'pld_score', keep = keep_gf_levels)
...
repr_df_all <- pld_vuln_gf %>%
  full_join(gen_time_gf %>% select(-db, -spec_code)) %>%
  full_join(max_fecund_gf %>% select(-db, -spec_code)) %>%
  full_join(parental_protect_gf %>% select(-db, -spec_code)) %>%
  full_join(parental_depend_gf %>% select(-db, -spec_code)) 

repr_df_drop_na <- repr_df_all %>%
  select(-db, -ends_with(c('_sd', '_nspp')), -spec_code) %>%
  distinct() %>%
  drop_na()
...
From this it appears that the first two principal components account for ~74% of the explainable variance in the data, one driven in large part by parental dependence (post-birth), and the other driven in large part by parental protection (pre-birth).
...
repr_out_df <- repr_df_all %>%
  select(db, spec_code, kingdom:species) %>%
  left_join(repr_pca_df) %>%
  mutate(db = ifelse(is.na(db), 'worms', db))

write_csv(repr_out_df, here('_data/traits_grouping/trait_reproduction_pca.csv'))
```
