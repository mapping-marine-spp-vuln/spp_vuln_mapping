...
fe_vs_hab_stack <- c(climate_map_hab,
                      nonclimate_map_hab,
                      climate_map_fe,
                      nonclimate_map_fe) %>%
  setNames(c('hab_cc', 'hab_noncc', 'fe_cc', 'fe_noncc'))

### Keep raster values with xy = TRUE, no need for cell IDs
hab_vs_fe_df <- as.data.frame(fe_vs_hab_stack, xy = TRUE)

### generate a LM based on climate stressors
cc_mdl <- lm(fe_cc ~ hab_cc + 0, data = hab_vs_fe_df)
ggplot(hab_vs_fe_df %>% sample_n(10000), aes(x = hab_cc, y = fe_cc)) +
  geom_point() +
...
noncc_mdl <- lm(fe_noncc ~ hab_noncc + 0, data = hab_vs_fe_df)
ggplot(hab_vs_fe_df %>% sample_n(10000), aes(x = hab_noncc, y = fe_noncc)) +
  geom_point() +
  geom_abline(intercept = 0,
...
$$I_{FE}^{cc} = 0.095 I_{hab}^{cc} + 0 \quad (adj.R^2 = 0.832)$$

$$I_{FE}^{non-cc} = 0.262 I_{hab}^{non-cc} + 0 \quad (adj.R^2 = 0.672)$$
