...
lgd_xy <- c(x = .15, y = 0.06, h = .22, w = .22)
p_w <- 0.55
...
x <- ggdraw() +
  ...
  ### Panel labels:
  draw_label('A.', x = a['x'], y = 1,  hjust = 0, vjust = 1, 
             size = 9, fontfamily = 'Helvetica', fontface = 'bold') +
  draw_label('B.', x = 1,  y = b['y'] + p_h -.01,  hjust = 1, vjust = 1, 
             size = 9, fontfamily = 'Helvetica', fontface = 'bold') +
  ### legend label:
  draw_label('Stressor count', x = lgd_xy['x'] + lgd_xy['w'] / 2, y = lgd_xy['y'] + lgd_xy['h'],
             hjust = .5, vjust = 1,
             size = 7, fontfamily = 'Helvetica', fontface = 'bold')

ggsave(fig6_file, height = plot_h, width = plot_w, units = 'cm', dpi = 300)
