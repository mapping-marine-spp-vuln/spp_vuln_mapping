...
tx_avg_med <- tx_imp_df %>%
  group_by(taxon, type, stressor, range) %>%
...
spp_imp_tots <- tx_imp_df %>%
  filter(type == 'NPP-weighted mean') %>%
  filter(stressor == 'all') %>%
  select(-stressor, -type, tot_imp = imp)
...
fig1_fourpanel <- ggdraw() +
  draw_plot(fig1, x = 0, y = .35, height = .65, width = 1) +
  draw_plot(panel_d, x = 0, y = 0, height = .35, width = 1)

fig1_fourpanel_file <- here('5_ms_figs/fig1_taxon_impact_summaries_fourpanel.png')
ggsave(fig1_fourpanel_file, width = 12, height = 9, units = 'cm', dpi = 300)
