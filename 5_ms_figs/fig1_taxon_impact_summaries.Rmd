...
if(!file.exists(spp_imp_sum_file)) { ### unlink(spp_imp_sum_file)
  spp_fs_valid <- assemble_spp_info_df() %>%
...
climate_strs <- c('sea_level_rise', 'uv_radiation', 'sst_extremes', 'sst_rise', 'ocean_acidification')
tx_imp_by_str <- spp_imp_by_str %>%
  ...
  mutate(str_lbl = fct_inorder(str_lbl),
         str_gp = ifelse(stressor %in% climate_strs, 'cc', 'noncc'))
...
panel_d <- ggplot(tx_imp_by_str, aes(x = mean_prop, y = taxon)) +
  geom_col(aes(fill = str_lbl, color = str_gp), size = 0.25) +
  scale_fill_manual(values = pal) +
                    #guide = guide_legend(ncol = 2)) +
  scale_color_manual(values = c('cc' = 'yellow'), na.value = NA, guide = FALSE) +
