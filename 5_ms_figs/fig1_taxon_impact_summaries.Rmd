...
if(!file.exists(spp_imp_sum_file)) { ### unlink(spp_imp_sum_file)
  spp_fs_valid <- assemble_spp_info_df() %>%
    ...
    group_by(species) %>%
    filter(src == 'am' | all(src == 'iucn')) %>%
      ### if any sources are AquaMaps, keep that; if all IUCN, keep those
      ### but honestly this probably does nothing lol
    ungroup()
...
if(!file.exists(spp_taxa_sum_file)) {
  spp_taxa <- assemble_spp_info_df() %>%
    ...
    mutate(taxon = str_replace(taxon, 'crustacea_arthropods', 'crustaceans'),
           ...
           taxon = str_replace(taxon, 'fish', 'ray-finned fishes'),
  ...
  p <- ggplot(df, aes(x = imp)) +
    ...
    ### clean up theme
      ...
      theme(panel.background = element_blank(),
            ...
            axis.title.x = element_text(face = 'bold', size = 8, hjust = 0)) +
...
fig1 <- ggdraw() +
  ...
  draw_label('B.', x = 0.405, y = .99, hjust = 0, vjust = 1, 
             size = 12, fontfamily = 'Arial', fontface = 'bold') +
  draw_label('C.', x = 0.66, y = .99, hjust = 0, vjust = 1,  
             size = 12, fontfamily = 'Arial', fontface = 'bold')
...
fig1_fourpanel_file <- here('5_ms_figs/fig1_taxon_impact_summaries.png')
ggsave(fig1_fourpanel_file, width = 6.25, height = 5, units = 'in', dpi = 300)
ggsave(fig1_fourpanel_file %>% str_replace('.png$', '.tif'), width = 6.25, height = 5, units = 'in', dpi = 300)
