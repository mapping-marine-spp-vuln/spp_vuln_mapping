...
panel_c <- ggplot(noncc_imp, aes(x = imp, y = taxon)) +
  ...
  geom_point(data = noncc_avg_med, aes(x = avg, group = type),
             ...
             show.legend = FALSE) +
  annotate('rect',
           xmin = .30, xmax = .37, ymin = 0.5, ymax = 13.5,
...
panel_c_plot <- panel_b + theme(legend.position = 'none')
panel_a_lgd  <- get_legend(panel_a)

### parameterize legends for easier changing of label positions:
lgd_a_xy <- c(x = .42, y = .12, h = .09, w = .12)

fig1 <- ggdraw() +
  ### draw panels:
  draw_plot(panel_a_plot,  x = 0.00, y = 0.00, height = 1.00, width = 0.40) +
  draw_plot(panel_b_plot,  x = 0.40, y = 0.00, height = 1.00, width = 0.28) +
  draw_plot(panel_c_plot,  x = 0.68, y = 0.00, height = 1.00, width = 0.32) +
  ### draw legends:
  ...
  draw_label('B', x = 0.40, y = .99, hjust = 0, vjust = 1, 
             size = 10, fontfamily = 'bentonbold')
  draw_label('C', x = 0.68, y = .99, hjust = 0, vjust = 1, 
             size = 10, fontfamily = 'bentonbold')
