...
vir_pal <- hcl.colors(n = 9, palette = 'Inferno', rev = TRUE)

colors_df <- crossing(x = 1:4, y = 1:4) %>%
  mutate(bi_class = paste(x, y, sep = '-')) %>%
  mutate(bi_fill = case_when(bi_class %in% c('1-2', '1-3', '2-1', '3-1') ~ '#eeeeff',
                             bi_class %in% c('2-2', '2-3', '3-2', '3-3') ~ '#ddddee',
                             bi_class %in% c('4-2', '4-3', '2-4', '3-4') ~ '#ccccdd',
                             bi_class == '1-1' ~ vir_pal[1], ### pale yellow; was ffffee
                             bi_class == '1-4' ~ vir_pal[4], ### red
                             bi_class == '4-1' ~ vir_pal[7], ### purple
