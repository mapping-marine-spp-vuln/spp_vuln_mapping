...
#        "PurpleGrn", "PurpleOr")
# bi_pal(x[11], dim = 4, preview = TRUE)

# pal_raw <- biscale:::bi_pal_pull(pal = 'PurpleGrn', dim = 4, flip_axes = FALSE, rotate_pal = FALSE)
# 
# colors_df <- data.frame(bi_class = names(pal_raw), bi_fill = pal_raw) %>%
...
#                              bi_class == '4-4' ~ bi_fill,
#                              TRUE ~ colorspace::lighten(bi_fill, amount = .1, space = 'HCL')))

vir_pal <- hcl.colors(n = 9, palette = 'Viridis', rev = TRUE)
### use 1, 3, 7, and 9 as seeds for corners; fade the upper left and lower right to
...
colors_df <- crossing(x = 1:4, y = 1:4) %>%
  ...
  mutate(bi_fill = case_when(bi_class == '1-1' ~ '#ffffee',
                             ...
                             bi_class %in% c('2-3', '3-2') ~ '#ccccdd',
                             bi_class %in% c('3-3') ~ '#bbbbcc',
                             bi_class %in% c('1-4') ~ colorspace::lighten(vir_pal[3], .2, space = 'HCL'), ### red
                             bi_class %in% c('2-4') ~ vir_pal[3], ### red
                             bi_class %in% c('3-4') ~ colorspace::darken(vir_pal[3], .2), ### red
                             bi_class %in% c('4-1') ~ colorspace::lighten(vir_pal[7], .4), ### purple
                             bi_class %in% c('4-2') ~ colorspace::lighten(vir_pal[7], .2), ### purple
                             bi_class %in% c('4-3') ~ vir_pal[7], ### purple
                             bi_class == '4-4' ~ vir_pal[9], ### black
                             TRUE ~ 'blue'))
