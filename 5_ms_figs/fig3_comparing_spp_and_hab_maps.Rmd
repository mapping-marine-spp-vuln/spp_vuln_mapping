...
nspp_mask <- rast(here('_output/nspp_maps/species_richness'))
nspp_vals <- values(nspp_mask)
values(nspp_mask)[nspp_vals < 20] <- NA

chi_gps_hab <- list.files(here('_output/impact_maps/cumulative_maps'),
...
x <- c("Bluegill", "BlueGold", "BlueOr", "BlueYl",
       ...
       "PurpleGrn", "PurpleOr")
# bi_pal(x[3], dim = 4, preview = TRUE)
# bi_pal(x[8], dim = 4, preview = TRUE)
# bi_pal(x[10], dim = 4, preview = TRUE)
# bi_pal(x[11], dim = 4, preview = TRUE)
# bi_pal(x[12], dim = 4, preview = TRUE)
...
### DF to plot frames around hot-hot and cool-cool legend
colors2_df <- crossing(x = 1:4, y = 1:4) %>%
  mutate(clr = case_when(x == 1 & y == 1 ~ 'green',
                         x == 4 & y == 4 ~ 'yellow',
                         x == 1 & y == 4 ~ 'red',
                         x == 4 & y == 1 ~ 'blue',
...
panel_bc_lgd_raw <- bi_legend(pal = pal_name, dim = 4,
                         xlab = "CHI hab -->",
                         ylab = "CHI FE -->",
                         size = 15,
                         arrows = FALSE) +
  ### add a new unfilled but color-framed tile on top
  geom_tile(data = colors2_df, aes(x, y, color = clr), 
            fill = NA, size = .5, show.legend = FALSE) +
  scale_color_manual(values = c('green', 'yellow', 'red', 'blue'), 
                     breaks = c('green', 'yellow', 'red', 'blue'), na.value = NA) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme(text = element_text(family = 'bentonbold'),
        plot.background = element_blank())
...
fig_3 <- ggdraw() +
  ...
  draw_label('Non-climate ->', 
             ...
             size = 20, color = 'black', fontfamily = 'bentonbold') +
  ### panel B & C labels:
  draw_label('FE vs. hab: cc/non-cc\nimpacts (quartiles)',
