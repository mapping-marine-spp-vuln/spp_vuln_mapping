...
# nspp_df <- big_df %>%
#   data.table() %>%
#   .[, .(nspp = sum(v_nspp)), by = .(cell_id)]
# rast_nspp <- map_to_mol(nspp_df, which = 'nspp')
# main_nspp_rast <- raster(here('_output/nspp_maps/nspp_in_unwt_vuln.tif'))
# compareRaster(rast_nspp, main_nspp_rast, values = TRUE, stopiffalse = TRUE)
# x <- calc(stack(-rast_nspp, main_nspp_rast), sum, na.rm = TRUE) %>% mask(ocean_a)
### difference of 1 spp around Caribbean and SW atlantic... ???
...
tmp_stem_fv <- here('tmp', 'impact_fv_chunks',
...
if(any(!file.exists(rast_mean_impact_fv_f, rast_sdev_impact_fv_f))) {
  ...
  mean_rast_fvwt <- map_to_mol(impact_map_df, which = 'fv_wt_mean_impact')
  sdev_rast_fvwt <- map_to_mol(impact_map_df, which = 'fv_wt_sd_impact')
  ...
  ### n_fe maps match well except one spot:
  n_fe_rast_main <- raster(here('_output/nspp_maps/n_fe_in_fvwt_vuln.tif'))
  n_fe_rast_fvwt <- map_to_mol(impact_map_df, which = 'n_fe')
  x <- stack(n_fe_rast_fvwt, -n_fe_rast_main) %>%
    calc(sum, na.rm = TRUE)
  values(x)[values(x)==0] <- NA
  plot(x)
  
  x_df <- data.frame(x = values(x),
                     cell_id = 1:ncell(x)) %>%
    filter(!is.na(x))
