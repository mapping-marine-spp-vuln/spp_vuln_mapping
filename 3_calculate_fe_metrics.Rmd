...
if(!file.exists(fe_metrics_file)) {
  ...
  am_spp_nonfe <- am_spp_info %>%
    filter(!species %in% spp_fe$species_am)
  table(am_spp_nonfe$class)
  worms_spp <- assemble_worms() 
  worms_am <- worms_spp %>%
    full_join(am_to_worms_fixes, by = c('species' = 'species_worms'))
  am_spp_nonworms <- am_spp_info %>%
    filter(!species %in% worms_spp$species & !species %in% am_to_worms_fixes$species_am)
  ...
  mammals <- am_spp_info %>%
    ...
    select(am_sid, species, species_am, fe_id) %>%
