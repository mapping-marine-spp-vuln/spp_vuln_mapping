...
```{r}
length_gf_order <- length_fb_gf %>%
  filter(!gf_level %in% c('unmatched', 'class'))

length_gf_summary <- length_gf_order %>%
  group_by(db, kingdom, phylum, class, gf_level) %>%
  summarize(n = n(), .groups = 'drop') %>%
  mutate(gf_level = paste0('gf_', gf_level),
         gf_level = factor(gf_level, levels = c('gf_match', 'gf_genus', 'gf_family', 'gf_order'))) %>%
  spread(gf_level, n)
...
results_df <- length_fb_gf %>%
  select(db, spec_code, species, log_l, log_l_sd, log_l_nspp, gf_level) %>%
  mutate(gf_level = factor(gf_level, levels = c('match', 'genus', 'family', 'order')),
         gf_level = as.integer(gf_level)) %>%
  filter(!is.na(gf_level))
