...
str_vuln_lookup <- tribble(
  ~stressor,                   ~vulnerability,
   ...
   'ocean_acidification_2020',  'ocean_acidification',
   'sea_level_rise_2019',       'sea_level_rise',
   'uv_radiation_2020',         'uv_radiation',
...
spp_wcol <- read_csv(here('_data/traits_grouping/trait_water_col_position.csv'), 
...
surface_npp <- raster(here_anx('stressors/fishing/npp', 'log_npp_present_surf_gf_mol.tif'))
benthic_npp <- raster(here_anx('stressors/fishing/npp', 'log_npp_present_benth_gf_mol.tif'))
npp_df <- data.frame(surface_npp = values(surface_npp),
                     benthic_npp = values(benthic_npp),
...
summarize_spp_impact <- function(spp_str_intsx, area_df, spp_name) {
  ...
  spp_intsx_area_df <- spp_str_intsx %>%
    ...
    mutate(intensity = intensity * ocean_a_km2, ### "intensity" in stressor per 100 km^2
           ...
           impact_npp = intensity * score * npp)
  ...
  imp_cum <- spp_intsx_area_df %>%
    summarize(vuln = 'cumulative', stressor = 'cumulative', 
              ...
              intensity_cv   = sd(intensity) / mean(intensity),
              impact_tot     = sum(impact), 
              impact_cv      = sd(impact[score > 0]) / mean(impact[score > 0]),
              impact_npp_tot = sum(impact_npp), 
              impact_npp_cv  = sd(impact_npp[score > 0]) / mean(impact_npp[score > 0]),
  ...
  for(k in 1:n_chunks) {
    ...
    tmp <- parallel::mclapply(seq_along(chunk_spp_vec),
        ...
        imp_full_range <- summarize_spp_impact(spp_str_intsx, ocean_a_df, spp) %>%
          mutate(range = 'full')
        imp_coastal_range <- summarize_spp_impact(spp_str_intsx, coastal_a_df, spp) %>%
          mutate(range = 'coastal')
        
        ### bind rows, add total and coastal ranges, and write out
        nonzero_score_stressors <- spp_am_vuln %>%
          filter(score > 0) %>%
          select(vuln, stressor, vuln_score = score) %>%
          bind_rows(data.frame(vuln = 'cumulative', stressor = 'cumulative', vuln_score = 1))
          
        spp_impacts_df <- bind_rows(imp_full_range, imp_coastal_range) %>%
          inner_join(nonzero_score_stressors, by = c('vuln', 'stressor')) %>%
  ...
  for(k in 1:n_chunks) {
    ...
    tmp <- parallel::mclapply(seq_along(chunk_spp_vec),
        ...
        imp_full_range <- summarize_spp_impact(spp_str_intsx, ocean_a_df, spp_name) %>%
          mutate(range = 'full')
        imp_coastal_range <- summarize_spp_impact(spp_str_intsx, coastal_a_df, spp_name) %>%
          mutate(range = 'coastal')
        
        ### bind rows, add total and coastal ranges, and write out
        nonzero_score_stressors <- spp_iucn_vuln %>%
          filter(score > 0) %>%
          select(vuln, stressor, vuln_score = score) %>%
          bind_rows(data.frame(vuln = 'cumulative', stressor = 'cumulative', vuln_score = 1))
