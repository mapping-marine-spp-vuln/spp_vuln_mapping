...
npp_df <- data.frame(surface_npp = as.vector(values(surface_npp)),
                     benthic_npp = as.vector(values(benthic_npp)),
...
if(n_chunks > 0) {
  for(k in 1:n_chunks) {
    ...
    tmp <- parallel::mclapply(seq_along(chunk_spp_vec),
        ...
        imp_full_range  <- summarize_spp_impact(spp_str_intsx, ocean_a_df, spp) %>%
          mutate(range = 'full')
        imp_coast_range <- summarize_spp_impact(spp_str_intsx, coastal_a_df, spp) %>%
