---
title: "Map FV-weighted IUCN extinction risk"
...
Apply extinction risk categories (by linear score, 0 = LC, 1 = EX) to IUCN-assessed spp in this analysis.  Calculate FV-weighted extinction risk across all FEs (gapfilling NAs to the mean value for the cell).  Also calculate unweighted for comparison, as well as standard deviations.
...
How to address data-deficient or unassessed species?  Fill with mean across all assessed spp in the cell (equally weighted)?

```{r function for processing extinction risk}

mc_process_ext_risk_chunk <- function(spp_maps_df, spp_info_df) {
  ...
  chunk_spp_ext_risk_maps <- chunk_maps %>%
  ...
  chunk_risk_sum_list <- parallel::mclapply(cell_gps, mc.cores = 25,
          FUN = function(gp) { ### gp <- 1
            ...
            df_unwt <- chunk_spp_ext_risk_maps %>%
              filter(cell_id %in% cell_ids) %>%
              data.table() %>%
              ### impute NAs using mean across all spp in cell
              .[ , cat_score := ifelse(is.na(cat_score), mean(cat_score, na.rm = TRUE), cat_score),
                 by = .(cell_id)] %>%
              ### Summarize unweighted mean and sd across all spp; na.rm unnecessary
              .[ , .(unwt_mean_risk = mean(cat_score),
                     unwt_sdev_risk = sd(cat_score)),
                 by = .(cell_id)] 
            
            df <- chunk_spp_ext_risk_maps %>%
              filter(cell_id %in% cell_ids) %>%
              data.table() %>%
              ### impute NAs using mean across all spp in cell
              .[ , cat_score := ifelse(is.na(cat_score), mean(cat_score, na.rm = TRUE), cat_score),
              ...
              .[ , .(score_mean = mean(cat_score),
                     score_sd   = sd(cat_score),
              ...
              .[ , .(n_fe = n_distinct(fe_id),
                     fv_mean = mean(fv),
                     fvwt_mean_risk = Hmisc::wtd.mean(score_mean,     weights = fv),
                     fvwt_sdev_risk = Hmisc::wtd.var(score_mean, weights = fv) %>%
                                         sqrt() %>% round(10)),
                 by = .(cell_id)] %>%
              ### bind unweighted df
...
For each chunk of cells, save out temp files of extinction risk metrics - fv-weighted mean, fv-weighted sd, number of functional entities, mean functional vulnerability across FEs - to be combined later.
...
tmp_stem <- here('tmp/ext_risk_fe_wt_chunks/ext_risk_summary_chunk_%s_to_%s.csv')
...
for(chunk_i in 1:n_chunks) { 
  ...
  if(file.exists(tmp_f)) {
    message('Extinction risk summaries exist for ', chunk_text, '... skipping!')
    next()
  }
  ...
  chunk_risk_sum <- mc_process_ext_risk_chunk(chunk_maps, spp_info_df)
...
chunk_sum_df <- data.frame(f = list.files(dirname(tmp_stem), full.names = TRUE,
                                          pattern = 'ext_risk_summary_chunk'))

fvmean_out_file <- here('_output/risk_maps/ext_risk_fvwt_mean.tif')
fvsdev_out_file <- here('_output/risk_maps/ext_risk_fvwt_sdev.tif')
unmean_out_file <- here('_output/risk_maps/ext_risk_unwt_mean.tif')
unsdev_out_file <- here('_output/risk_maps/ext_risk_unwt_sdev.tif')

### Combine mean, sdev, and n_fe maps by taxon into one big dataframe
message('Processing mean, sd maps across all species for extinction risk...')
...
fvmean_rast <- map_to_mol(stats_df, which = 'fvwt_mean_risk')
fvsdev_rast <- map_to_mol(stats_df, which = 'fvwt_sdev_risk')
unmean_rast <- map_to_mol(stats_df, which = 'unwt_mean_risk')
unsdev_rast <- map_to_mol(stats_df, which = 'unwt_sdev_risk')
...
ggplot(fvmean_df, aes(x, y, fill = fvwt_mean_risk)) +
  geom_raster() +
  scale_fill_viridis_c() +
  theme_void() +
  coord_sf() +
  labs(title = 'FV-weighted Mean IUCN extinction risk')
ggplot(eqmean_df, aes(x, y, fill = unwt_mean_risk)) +
  geom_raster() +
  scale_fill_viridis_c() +
  theme_void() +
  coord_sf() +
  labs(title = 'Equal-weighted Mean IUCN extinction risk')
...
for(r in rich_vec) {
  raw_df <- data.frame(i = rep(1:sims, each = r)) %>%
    mutate(cat = sample(spp_info_df$cat_score, size = n(), replace = TRUE)) %>%
    group_by(i) %>%
    mutate(spp = 1:n(),
