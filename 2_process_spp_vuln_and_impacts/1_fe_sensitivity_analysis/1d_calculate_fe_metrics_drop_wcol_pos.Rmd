...
spp_info <- assemble_spp_info_df() %>%
  ...
  distinct() %>%
  mutate(fe = paste(mob, len, trp, sep = '_')) %>%
  group_by(fe) %>%
  mutate(n_fe = n()) %>%
  ungroup() %>%
  arrange(desc(n_fe)) %>%
...
fe_summary_file <- here_anx('func_entities', 'sens_analysis', 'fe_drop_wcol_pos_cell_summary.csv')
...
if(!file.exists(fe_summary_file)) {
  ...
  tmp_chunk_filestem <- here('tmp/fe_drop_wcol_pos_summary_chunk_%s.csv')
  ...
  fe_tmp_fs <- list.files(dirname(tmp_chunk_filestem), 
                          pattern = 'fe_drop_wcol_pos_summary_chunk', full.names = TRUE)
...
fe_metrics_file <- here_anx('func_entities/sens_analysis/fe_drop_wcol_pos_metrics_per_cell.csv')
...
writeRaster(n_fe_rast,    here('_output/func_entities/sens_analysis/n_fe_drop_wcol_pos.tif'),    
            overwrite = TRUE)
writeRaster(f_wvuln_rast, here('_output/func_entities/sens_analysis/f_wvuln_drop_wcol_pos.tif'), 
            overwrite = TRUE)
writeRaster(f_red_rast,   here('_output/func_entities/sens_analysis/f_red_drop_wcol_pos.tif'),   
            overwrite = TRUE)
writeRaster(f_overred_rast, here('_output/func_entities/sens_analysis/f_overred_drop_wcol_pos.tif'), 
            overwrite = TRUE)
```
