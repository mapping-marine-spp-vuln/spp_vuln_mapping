...
impact_map_stem <- here('_output/impact_maps/impact_maps_unweighted', 
                        'impact_unwt_biomass_removal_%s.tif')
...
plot(log10(mean_rast_unwt), col = map_cols, #zlim = c(0, 1),
     main = 'log_10(Mean) unweighted impact: biomass removal',
     legend = TRUE, axes = FALSE)  
plot(cv_rast_unwt, col = map_cols, 
     main = 'CV unweighted weighted impact: biomass removal',
     legend = TRUE, axes = FALSE)
...
### zxcv <- list.files(dirname(tmp_stem_fv), pattern = 'imp_fvwt', full.names = TRUE)
### unlink(zxcv)
...
for(chunk_i in 1:n_chunks) { 
  ...
  if(nrow(chunk_catch_maps_raw) == 0 & nrow(chunk_rangemaps_raw) > 0) {
    ...
    range_maps_fv <- calc_spp_cell_fv(spp_cells = chunk_rangemaps_raw,
                                      spp_fe = spp_fe)

    no_catch_df <- range_maps_fv %>%
      group_by(cell_id) %>%
  ...
  message('... Calculating functional vulnerability metrics for ', nrow(catch_maps_filled), 
          ' spp-cells in \n    ', chunk_text, '...')
  chunk_maps_fv <- calc_spp_cell_fv(spp_cells = catch_maps_filled,
                                    spp_fe = spp_fe)
...
rast_fv_out_stem <- here('_output/impact_maps/impact_maps_fv_weighted', 
                         'impact_fvwt_biomass_removal_%s.tif')
...
high_diff_cells <- data.frame(unwt = as.vector(values(mean_rast_unwt)), 
                              fvwt = as.vector(values(mean_rast_fvwt)),
