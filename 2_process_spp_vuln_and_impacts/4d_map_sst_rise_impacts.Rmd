...
# main_nspp_rast <- rast(here('_output/nspp_maps/nspp_in_unwt_vuln.tif'))
...
plot(mean_rast_unwt, col = map_cols, 
...
### zxcv <- list.files(dirname(tmp_stem_fv), pattern = 'impact_summary_chunk', full.names = TRUE)
### unlink(zxcv)
...
```{r}
mean_rast_fvwt <- rast(rast_mean_impact_fv_f)
sdev_rast_fvwt <- rast(rast_sdev_impact_fv_f)
cv_rast_fvwt <- sdev_rast_fvwt / mean_rast_fvwt

map_cols <- hcl.colors(n = 50)

plot(mean_rast_fvwt, col = map_cols, 
