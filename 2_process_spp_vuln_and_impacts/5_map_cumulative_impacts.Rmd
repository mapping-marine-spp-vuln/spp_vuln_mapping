...
writeRaster(chi_unweighted, here('_output/impact_maps/cumulative_maps/chi_unweighted.tif'),
            overwrite = TRUE)
...
writeRaster(chi_fv_weighted, here('_output/impact_maps/cumulative_maps/chi_fv_weighted.tif'),
            overwrite = TRUE)
...
mean_str_unwt <- sapply(imp1_stack, 
                        FUN = function(r) mean(values(r), na.rm = TRUE)) %>%
  setNames(names(imp1_stack))
mean_str_unwt_coastal <- sapply(imp1_stack %>% mask(neritic_rast), 
                                FUN = function(r) mean(values(r), na.rm = TRUE)) %>%
  setNames(names(imp1_stack))

mean_str_fvwt <- sapply(imp2_stack, 
                        FUN = function(r) mean(values(r), na.rm = TRUE)) %>%
  setNames(names(imp2_stack))
mean_str_fvwt_coastal <- sapply(imp2_stack %>% mask(neritic_rast), 
                                FUN = function(r) mean(values(r), na.rm = TRUE)) %>%
  setNames(names(imp2_stack))

mean_str_df <- data.frame(stressor = names(imp1_stack)) %>%
  mutate(mean_impact_fvwt = mean_str_fvwt[stressor],
         mean_impact_fvwt_nonzero = mean_str_fvwt_coastal[stressor],
         mean_impact_unwt = mean_str_unwt[stressor],
         mean_impact_unwt_nonzero = mean_str_unwt_coastal[stressor]) %>%
