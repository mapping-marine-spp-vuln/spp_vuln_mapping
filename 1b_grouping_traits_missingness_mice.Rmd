---
title: "Examine missingness of grouping traits for MICE"
...
Read in processed traits data as a dataframe.  Identify missingness, overall and by taxonomic grouping, to identify challenges in using MICE for imputation of missing values.
...
#### Worms/AquaMaps/traits species counts: by class for Mollusca

Because only ~half the molluscs with grouping traits are represented in AquaMaps (and only conidae in IUCN), drop any orders with fewer than five species in AquaMaps.  This should reduce the computational load.

``` {r}
n_molluscs_all <- worms_premice %>%
  filter(phylum == 'mollusca') ### 10232 mollusc spp represented

worms_am_premice %>% 
  filter(phylum == 'mollusca') %>% ### 4907 represented in AquaMaps; only conus in IUCN
...
* Dropping phyla whose traits data and AquaMaps intersection results in fewer than 75 species.  
    ...
    * Of the remaining, include class or order or family as additional gapfilling variables, rather than further subdividing the phylum.
    * For Molluscs, which fail in `mice` due to too many possible categorical values at phylum level:
        * Identify and filter the full trait set to just those orders comprising at least five species.
...
invert_keep <- worms_am_premice %>%
  filter(phylum != 'chordata' & phylum != 'mollusca') %>%
  group_by(phylum) %>%
  summarize(n_spp = n_distinct(species)) %>%
  filter(n_spp >= 75) %>%
  pull(phylum)

mollusc_keep <- worms_am_premice %>%
...
keep_df <- am_spp %>%
  mutate(keep = case_when(class %in% names(vert_keep) ~ TRUE,
                          phylum %in% invert_keep ~ TRUE,
                          phylum == 'mollusca' & order %in% mollusc_keep ~ TRUE,
                          TRUE ~ FALSE)) %>%
  filter(keep) %>%
...
write_csv(keep_df, here('int/mice_grouping_taxa_to_keep.csv'))
```
...
#### Worms/traits species counts: by class for Mollusca, after dropping
...
#### Invertebrate phyla (sans molluscs)

```{r, results = 'asis'}
invert_traits <- premice_keep %>%
...
```{r, results = 'asis'}
mollusc_traits <- premice_keep %>%
  filter(phylum == 'mollusca') %>%
  mutate(order %in% mollusc_keep) %>%
  mutate(gp = class)

for(c in unique(mollusc_traits$gp)) {
  ### c <- mollusc_keep[1]
  class_traits <- mollusc_traits %>%
    filter(gp == c) %>%
    select(log_l, log_f, troph, age_mat, wcol, adult_mob) 
