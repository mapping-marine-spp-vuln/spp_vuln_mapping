...
wcol_traits <- read_csv(here('_data/traits_grouping/trait_water_col_position.csv')) %>%
  select(species, wcol = value) %>%
  mutate(wcol = ifelse(wcol %in% c('benthopelagic', 'reef'), 'both', wcol))


spp_for_imp_calc <- all_spp %>%
  filter(!is.na(score)) %>%
  ### mapped in AquaMaps and/or IUCN
  filter(am_mapped | iucn_mapped) %>%
  left_join(wcol_traits, by = 'species')

```

Read in bycatch stressor maps, and create a dataframe of the results.  Assign species to one of three bins based on water column position trait.  Also, set up a function for reading in rangemaps.

```{r}
benth_rast <- raster(here('_data/stressors_mol/bycatch_benthic_2017.tif'))
pelag_rast <- raster(here('_data/stressors_mol/bycatch_pelagic_2017.tif'))
bp_rast <- (benth_rast + pelag_rast) / 2

bycatch_cells_df <- data.frame(benthic = values(benth_rast),
                         pelagic = values(pelag_rast),
                         both    = values(bp_rast),
                         cell_id = 1:ncell(benth_rast)) %>%
  filter(!is.na(benthic) | !is.na(pelagic)) %>%
  pivot_longer(names_to = 'wcol', values_to = 'bycatch_intensity', cols = -cell_id)

rangemap_stem <- here_anx('spp_maps_mol/%s_spp_mol_%s.csv')
mapfile_df <- spp_for_imp_calc %>%
  select(species, am_mapped, iucn_mapped, iucn_sid) %>%
  distinct() %>%
  mutate(src   = ifelse(iucn_mapped, 'iucn', 'am'),
         spp   = str_replace_all(species, ' ', '_'),
         id    = ifelse(src == 'iucn', iucn_sid, spp),
         map_f = sprintf(rangemap_stem, src, id)) %>%
  select(species, src, map_f)
```

``` {r}
get_rangemap <- function(f) {
  if(!file.exists(f)) stop('Spp rangefile does not exist: ', basename(f))
  ### set up for multi range files in case of IUCN spp with subpops
  src <- str_remove(basename(f)[1], '_spp_mol_.+')
  df <- lapply(f, data.table::fread) %>%
...
out_stem <- here_anx('impact_maps_by_taxon/impacts_tx_bycatch', 
                     'imp_unweighted_bycatch_all',
                     'imp_unweighted_bycatch_%s_%s.tif')
...
for(t in taxa) {
  ...
  tx_maps_list <- parallel::mclapply(
                    tx_mapfiles$map_f, ### f <- tx_mapfiles$map_f[1]
                    mc.cores = 40, 
                    FUN = get_rangemap)
  ...
  message('... Binding range maps for taxon ', t, '...')
  ...
  message('Processing mean/sd vulnerability by species in taxon ', t, ' to bycatch stressor...')
  ...
  result_list <- parallel::mclapply(1:n_chunks, mc.cores = n_cores,
                 FUN = function(n) { ### n <- 1
                   ...
                   chunk_sum <- tx_maps %>%
                     ...
                     oharac::dt_join(tx_imp_df, by = 'species', type = 'left') %>%
                     oharac::dt_join(bycatch_cells_df, by = c('cell_id', 'wcol'), type = 'inner') %>%
                     mutate(impact = score * bycatch_intensity) %>%
                     group_by(cell_id) %>%
  ...
  if(check_tryerror(result_list)) {
    warning('Something went wrong with calculations for taxon ', t, '... skipping for now!')
    next()
  }
...
tx_bycatch_map_df <- data.frame(f = list.files(dirname(out_stem), full.names = TRUE)) %>%
...
impact_map_stem <- here('_output/impact_maps/impact_maps_unweighted_all', 
                        'impact_unweighted_bycatch_%s.tif')
...
Loop over each taxon; pull all max temp stressor files for that taxon.  For each species in the taxon, multiply bycatch stressor map by the spp vulnerability to identify impact map for that species.  Summarize across the entire taxon to mean, sd, and nspp.
...
out_stem_fe <- here_anx('impact_maps_by_taxon/impacts_tx_bycatch', 
                        'imp_unweighted_bycatch_fe_only',
                        'imp_unweighted_bycatch_fe_%s_%s.tif')
### zxcv <- list.files(dirname(out_stem_fe), full.names = TRUE)
### unlink(zxcv)
for(t in taxa) {
  ### t <- taxa[1]
  tx_imp_df <- spp_for_imp_fe_only %>%
    filter(taxon == t) %>%
    select(species, score, wcol) %>%
    distinct()
  ...
  message('Loading range maps for taxon ', t, '...')
  tx_maps_list <- parallel::mclapply(
                    tx_mapfiles$map_f,
                    mc.cores = 40, 
                    FUN = get_rangemap)
  ...
  message('... Binding range maps for taxon ', t, ' FE species...')
  tx_maps <- tx_maps_list %>%
    setNames(tx_mapfiles$species) %>%
    data.table::rbindlist(idcol = 'species')
  
  message('... Taxon ', t, ' bycatch stressor dataframe: ', nrow(tx_maps), 
          ' cell observations for ', n_distinct(tx_mapfiles$species), 
          ' FE species...')
  
  message('Processing mean/sd vulnerability by FE species in taxon ', t, ' to bycatch stressor...')
...
fe_tx_bycatch_map_df <- data.frame(f = list.files(dirname(out_stem_fe), full.names = TRUE)) %>%
  mutate(t = str_extract(basename(f), paste0(taxa, collapse = '|')),
         p = str_extract(basename(f), '_mean|_sdev|_nspp') %>% str_remove('_'))

impact_map_stem_fe <- here('_output/impact_maps/impact_maps_unweighted_fe_only', 
                        'impact_unweighted_fe_only_bycatch_%s.tif')
  ### %s is parameter (mean, sd, nspp)
...
if(!all(file.exists(impact_f_mean, impact_f_sdev))) {
  ### Combine mean, sdev, and nspp maps by taxon into one big dataframe
  message('Processing mean, sd, nspp maps across FE species for bycatch stressor...')
  big_df_fe <- combine_taxa_maps(fe_tx_bycatch_map_df)
}
...
if(!file.exists(impact_f_mean)) {
  ...
  fe_spp_mean <- process_mean_rasts(big_df_fe)
  rast_mean <- map_to_mol(fe_spp_mean, which = 'vuln_mean')
...
if(!file.exists(impact_f_sdev)) {
  ...
  fe_spp_sdev <- process_sdev_rasts(big_df_fe)
  rast_sdev <- map_to_mol(fe_spp_sdev, which = 'vuln_sdev')
...
plot(log10(mean_rast_unwt), col = map_cols, 
     main = 'log_10(Mean) unweighted impact: bycatch (FE spp only)',
     legend = TRUE, axes = FALSE)  
plot(cv_rast_unwt, col = map_cols, 
     main = 'CV unweighted weighted impact: bycatch (FE spp only)',
     legend = TRUE, axes = FALSE)
```
...
spp_for_imp_fv <- spp_for_imp_calc %>%
  inner_join(spp_fe, by = 'species')

tmp_stem_fv <- here_anx('impact_maps_by_taxon/impacts_tx_bycatch', 'imp_fv_weighted_bycatch',
                        'impact_summary_chunk_%s_to_%s_bycatch.csv')
### zxcv <- list.files(dirname(tmp_stem_fv), full.names = TRUE)
### unlink(zxcv)

mapfile_fv_df <- mapfile_df %>%
  filter(species %in% spp_for_imp_fv$species) %>%
  distinct()
...
for(chunk_i in 1:n_chunks) { 
  ...
  message('Loading ', nrow(mapfile_fv_df), ' rangemaps cropped for ', chunk_text,  '...')
  
  chunk_maps_list <- parallel::mclapply(mapfile_fv_df$map_f, mc.cores = 30, 
                                        ### f <- mapfile_fe_df$map_f[1]
                                        FUN = read_truncated_rangemap, 
                                        chunk_start = chunk_start, chunk_end = chunk_end) 
  
  chunk_maps_raw <- bind_maps_list(chunk_maps_list, mapfile_fv_df)
  
  ### if result includes no cells, write out empty chunk files
  if(nrow(chunk_maps_raw) == 0) {
    message('No species X cells found for ', chunk_text, '... writing out empty data.frame...')
  ...
  chunk_str_sum <- calc_chunk_str_sum(chunk_maps, spp_for_calc = spp_for_imp_fv)
...
rast_fv_out_stem <- here('_output/impact_maps/impact_maps_fv_weighted', 
                         'impact_fv_weighted_bycatch_%s.tif')
...
```{r}
mean_rast_fvwt <- raster(rast_mean_imp_fv_f)
sdev_rast_fvwt <- raster(rast_sdev_imp_fv_f)
