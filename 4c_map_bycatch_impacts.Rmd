...
bycatch_cells_df <- data.frame(ben = values(benth_rast),
                         pel = values(pelag_rast),
                         both    = values(bp_rast),
...
for(t in taxa) {
  ...
  if(check_tryerror(result_list)) {
    warning('Something went wrong with calculations for taxon ', t,
            '... skipping for now!')
    next()
  }
...
if(!file.exists(impact_f_mean)) {
  ...
  rast_mean <- map_to_mol(unwt_mean, which = 'vuln_mean')
...
if(!file.exists(impact_f_sdev)) {
  ...
  rast_sdev <- map_to_mol(unwt_sdev, which = 'vuln_sdev')
...
# unwt_nspp <- big_df %>% group_by(cell_id) %>% summarize(nspp = sum(v_nspp))
# rast_nspp <- map_to_mol(unwt_nspp, which = 'nspp')
# main_nspp <- raster(here('_output/nspp_maps/nspp_in_unwt_vuln.tif'))
# compareRaster(rast_nspp, main_nspp, values = TRUE)
```
...
if(any(!file.exists(rast_mean_imp_fv_f, rast_sdev_imp_fv_f))) {
  ...
  # main_n_fe <- raster(here('_output/nspp_maps/n_fe_in_fvwt_vuln.tif'))
  # compareRaster(rast_n_fe, main_n_fe, values = TRUE)
}
