...
check_df <- depth_vuln %>%
  filter(gf_level <= 3) %>%
  select(species, trait_v = trait, value_v = value, gf_level) %>%
  mutate(gf_level = as.character(gf_level)) %>%
  full_join(depth_fb %>% 
              filter(gf_level <= 3) %>%
              select(species, trait_fb = trait, value_fb = value))

ggplot(check_df, aes(x = value_v, y = value_fb, color = gf_level)) +
...
depth_df <- depth_vuln %>%
  select(species, value_v = value, gf_v = gf_level, nspp_v = nspp, sd_v = sd) %>%
  full_join(depth_fb %>% 
              select(species, value_fb = value, gf_fb = gf_level, nspp_fb = nspp, sd_fb = sd)) %>%
  mutate(hi_nspp = ifelse(nspp_v > nspp_fb, value_v, value_fb),
         trait = 'depth',
         depth_score = case_when(gf_v == 1     ~ value_v, ### if direct match, use it
                                 gf_fb < gf_v  ~ value_fb,   ### if fb closer to match, use it
                                 gf_v == gf_fb ~ hi_nspp,
                                 is.na(value_v) ~ value_fb,
                                 TRUE           ~ value_v),
         gf_level    = case_when(gf_v == 1     ~ gf_v, 
                                 gf_fb < gf_v  ~ gf_fb, 
                                 gf_v == gf_fb ~ gf_v,
                                 is.na(value_v) ~ gf_fb,
                                 TRUE           ~ gf_v),
         sd          = case_when(gf_v == 1     ~ sd_v,
                                 gf_fb < gf_v  ~ sd_fb,
                                 gf_v == gf_fb ~ sd_v,
                                 is.na(value_v) ~ sd_fb,
                                 TRUE           ~ sd_v), 
         nspp        = case_when(gf_v == 1     ~ nspp_v, 
                                 gf_fb < gf_v  ~ nspp_fb, 
                                 gf_v == gf_fb ~ nspp_v,
                                 is.na(value_v) ~ nspp_fb,
                                 TRUE           ~ nspp_v)) %>%
  select(species, trait, value = depth_score, gf_level, sd, nspp)
# x <- depth_df %>% filter(depth_vuln != depth_fb)

water_col_df <- depth_df %>%
  bind_rows(terr_vuln, vert_mig_vuln) 

# trait_check <- water_col_df %>%
#   filter(!is.na(value)) %>%
#   group_by(species) %>%
#   summarize(n_traits = n_distinct(trait)) %>%
#   ungroup()
# table(trait_check$n_traits)
#     1      2      3 
# 73609  11475 124653 

### for the spread() to work, collapse nspp and gf_level to mean values to avoid
### multiple values per species
water_col_dropna <- water_col_df %>%
  group_by(species) %>%
  mutate(nspp = mean(nspp), gf_level = mean(gf_level)) %>%
  ungroup() %>%
  select(-sd) %>%
  spread(trait, value) %>%
  # filter(gf_level <= 4) %>% ### keep order and lower
  full_join(get_worms()) %>%
  mutate(tx = ifelse(phylum == 'chordata', class, phylum)) %>%
  select(tx, species, depth, terr_score, vert_mig_score) %>%
  drop_na()
...
Keeping the first two components will capture 80% of the explained variation.  The first relates more heavily to water column position in the usual sense along with terrestrial/marine life stages; the second combines vertical migration score with terrestrial/marine life stages. 
...
results_df <- water_col_df %>%
  ...
  filter(!is.na(depth_pc1))

write_csv(results_df, here('_data/traits_grouping/trait_depth_raw.csv'))

pca1_df <- results_df %>%
  select(species, value = depth_pc1, gf_level) %>% mutate(trait = 'depth_pc1')
pca2_df <- results_df %>%
  select(species, value = depth_pc2, gf_level) %>% mutate(trait = 'depth_pc2')
write_csv(pca1_df, here('_data/traits_grouping/trait_depth_pc1.csv'))
write_csv(pca2_df, here('_data/traits_grouping/trait_depth_pc2.csv'))
