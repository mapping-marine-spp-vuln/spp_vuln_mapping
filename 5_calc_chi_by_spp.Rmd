...
for(k in 1:n_chunks) {
  ...
  tmp <- parallel::mclapply(seq_along(chunk_sid_vec),
    ...
    # for(i in seq_along(chunk_sid_vec)) {
      ...
      ### create a vector of species presence by cell area
      spp_ocean_vec <- spp_coastal_vec <- ocean_area_vec
      spp_ocean_vec[-spp_cells$loiczid] <- NA
      spp_coastal_vec[-(spp_cells %>% filter(coastal) %>% pull(loiczid))] <- NA
    
      ### Calculate species total range and coastal range
      range_tot_km2 <- sum(spp_ocean_vec, na.rm = TRUE)
      range_coastal_km2 <- sum(spp_coastal_vec, na.rm = TRUE)
      ...
      spp_impacts_df <- spp_impacts_list %>%
        bind_rows() %>%
        mutate(range_tot_km2     = range_tot_km2,
               range_coastal_km2 = range_coastal_km2)
