...
for(k in 1:n_chunks) {
  ...
  ptm <- proc.time()
    am_spp_cells_chunk <- am_spp_cells %>%
      filter(am_sid %in% chunk_sid_vec)
  ptm <- proc.time() - ptm
  message('Filtering ', length(chunk_sid_vec), ' species in chunk ', k, ' of ', 
          n_chunks, ': ', round(ptm[3], 3), 's elapsed time')
  ### 5 seconds for filtering 100 spp; 7.5 for filtering 1000
  ...
  # for(i in seq_along(chunk_sid_vec)) {
    ...
    ### this adds a couple of seconds to each iteration; chunk spp list for speed?
    # system.time({
    #   for(w in 1:10) {
    #     spp_cells <- am_spp_cells %>%
    #       filter(am_sid == id)
    #   }
    # }) ### 10 iterations over 100-member chunk or 1000-member chunk: 25 seconds
    
    # system.time({
    #   for(w in 1:10) {
    #     spp_cells2 <- am_spp_cells_chunk %>%
    #       filter(am_sid == id)
    #   }
    # }) ### 10 iterations over 100 member chunk: 0.134 seconds; .475 for 1000 member chunk
    ...
    message(i, ' of ', length(chunk_sid_vec), ', chunk ', k, ' of ', n_chunks, 
            ...
            '... elapsed time: ', round(proc.time() - ptm2, 3)[3], 's.')
  })
