...
vuln1_mean_fs <- list.files(here('_output/vuln_maps/vuln_maps_unweighted_all'), 
...
vuln1_mean_stack <- stack(vuln1_mean_fs) %>%
  setNames(basename(vuln1_mean_fs) %>% str_remove_all('vuln_all_spp_|_mean.tif'))

vuln1_sd_fs <- list.files(here('_output/vuln_maps/vuln_maps_unweighted_all'), 
...
vuln1_sd_stack <- stack(vuln1_sd_fs) %>%
  setNames(basename(vuln1_sd_fs) %>% str_remove_all('vuln_all_spp_|_sdev.tif'))
```
...
for(i in 1:nrow(str_vuln_lookup)) {
  ...
  f_stem <- here('_output/impact_maps/impact_maps_unweighted_all', 
                 'impact_unweighted_%s_x_%s_%s.tif')
...
imp_map_dir <- here('_output/impact_maps/impact_maps_unweighted_all')
...
vuln2_mean_stack <- stack(vuln2_mean_fs) %>%
  setNames(basename(vuln2_mean_fs) %>% str_remove_all('vuln_fv_wt_|_mean.tif'))
...
vuln2_sd_stack <- stack(vuln2_sd_fs) %>%
  setNames(basename(vuln2_sd_fs) %>% str_remove_all('vuln_fv_wt_|_sdev.tif'))
```
...
for(i in 1:nrow(str_vuln_lookup)) {
  ...
  f_stem <- here('_output/impact_maps/impact_maps_fv_weighted', 
                 'impact_fv_weighted_%s_x_%s_%s.tif')
  f_out  <- sprintf(f_stem, s, v, c('mean', 'sdev'))
...
## Impacts by unweighted vulnerability (FE species only)

Calc impacts by unweighted vuln, using only those spp with defined functional entities, to more appropriately compare differences when considering functional vulnerability vs. not

```{r}
vuln3_mean_fs <- list.files(here('_output/vuln_maps/vuln_maps_unweighted_fe_only'), 
                       pattern = 'mean.tif', full.names = TRUE)
vuln3_mean_fs <- vuln3_mean_fs[str_detect(vuln3_mean_fs, vuln_string)]

vuln3_mean_stack <- stack(vuln3_mean_fs) %>%
  setNames(basename(vuln3_mean_fs) %>% str_remove_all('vuln_fe_spp_|_mean.tif'))

vuln3_sd_fs <- list.files(here('_output/vuln_maps/vuln_maps_unweighted_fe_only'), 
                       pattern = 'sdev.tif', full.names = TRUE)
vuln3_sd_fs <- vuln3_sd_fs[str_detect(vuln3_sd_fs, vuln_string)]

vuln3_sd_stack <- stack(vuln3_sd_fs) %>%
  setNames(basename(vuln3_sd_fs) %>% str_remove_all('vuln_fe_spp_|_sdev.tif'))

```

```{r loop impact by unweighted vulnerability FE only}
for(i in 1:nrow(str_vuln_lookup)) {
  ### i <- 2
  s <- str_vuln_lookup$stressor[i]
  v <- str_vuln_lookup$vulnerability[i]
  
  f_stem <- here('_output/impact_maps/impact_maps_unweighted_fe_only', 
                 'impact_unweighted_fe_only_%s_x_%s_%s.tif')
  f_out  <- sprintf(f_stem, s, v, c('mean', 'sdev'))
  
  if(any(!file.exists(f_out))) {
    message('Calculating unweighted mean/sd impacts (FE spp only) for stressor ', s, 
            ' by vuln ', v, '...')
    
    s_rast <- str_stack[[s]]
    v_mean_rast <- vuln3_mean_stack[[v]]
    v_sdev_rast <- vuln3_sd_stack[[v]]
    i_mean_rast <- s_rast * v_mean_rast
    i_sdev_rast <- s_rast * v_sdev_rast
    # values(i_rast) <- round(values(i_rast), 5)
...
```{r plot impacts by unweighted vuln FE only, eval = FALSE, fig.height = 4, fig.width = 7}

imp_map_dir <- here('_output/impact_maps/impact_maps_unweighted_fe_only')
imp_map_fs <- list.files(imp_map_dir, pattern = '_mean.tif', full.names = TRUE)
...
for(f in focal_strs) { ### f <- focal_strs[1]
  ...
  plot(imp_rast, col = map_cols, 
       main = paste0('Impact, unweighted vuln (FE spp only): ', f),
       legend = TRUE, axes = FALSE)  
}
```
...
mean_unwt_fs <- list.files(here('_output/impact_maps/impact_maps_unweighted_fe_only'), 
