...
iucn_assessed <- read_csv(here('_data/iucn_spp/iucn_marine_spp_info_2021-3.csv')) %>%
  ...
  left_join(read_csv(here('_data/iucn_spp', 'iucn_to_worms_match.csv')), by = 'iucn_sid') %>%
  mutate(threatened = cat %in% c('vu', 'en', 'cr', 'ex')) %>%
  select(iucn_sid, iucn_sciname = sciname, sciname = worms_name, cat, cat_score, threatened)
...
assessed_in_am <- get_am_spp_info() %>%
  ...
  inner_join(iucn_assessed %>% 
               select(iucn_sid, cat, threatened) %>% distinct(), 
             by = 'iucn_sid')
