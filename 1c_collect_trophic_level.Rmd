...
```{r}
food_df <- get_fb_slb(fxn = fooditems, keep_cols = c('food_i', 'troph'), keep_fxn = contains) %>%
  distinct()
...
# troph_se_df_gf <- troph_df %>%
#   right_join(get_worms()) %>%
#   gapfill_up_down('troph_se') %>%
#   select(species, trait, value, sd, nspp, gf_level) %>%
#   filter(gf_level <= 4)

write_csv(troph_df_gf, here('_data/traits_grouping/trait_trophic_level.csv'))
# write_csv(troph_se_df_gf, here('_data/traits_grouping/trait_trophic_se.csv'))
